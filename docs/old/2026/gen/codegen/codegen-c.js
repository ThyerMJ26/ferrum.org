...
import { exprFreeVars } from "../syntax/expr.js";
import { locContains, showLoc } from "../syntax/token.js";
import { typeHd, typeTl, typeDom, anyT, ttiIsFalse, tiStructuralRelComp, strT, collectUnionTypes, pairT, intersectTypes, disjoinTypes, applyTypes, substType, knownInhabited, showType2, singleT } from "../tree/types.js";
import { mk_MemoData } from "../tree/memoize.js";
import { isAlphanum, isAlphanumUS } from "../syntax/scan.js";
import { evalNode } from "../tree/eval.js";
...
function tName(name) { return { tag: "TName", name: name }; }
function tPtr(ty) { return { tag: "TPtr", ty: ty }; }
function tArray(ty, len) { return { tag: "TArray", ty: ty, len: len }; }
function tConst(ty) { return { tag: "TConst", ty: ty }; }
function tUnion(fields) { return { tag: "TUnion", fields: fields }; }
let tAny = tName("Any");
let tStr = tName("Str");
let tType = tName("Type");
...
function erPrim(cb, natName, domRs, codR) {
...
function toAny(cb, er) {
    return reprConvertToAny(cb, er);
}
function toRepr(cb, repr, er) {
    return reprConvert(cb, repr, er);
}
function toVar(cb, er) {
    if (er.expr.tag === "CVar") {
...
function toVarAtDepth(cb, er, depth) {
    if (er.expr.tag === "CVar") {
        return er.expr;
    }
    else {
...
function reprConvertFromAny(cb, to, exp0) {
    let exp;
    ...
    switch (to.tag) {
        ...
        case "List": {
            let listRepr = createListRepr(cb, to.elemRepr);
...
function reprConvertToAny(cb, exp) {
    let from = exp.repr;
    switch (from.tag) {
        case "None":
...
function reprConvert(cb, to, exp) {
    let from = exp.repr;
...
function reprsToTypeName(depth, label, arity, reprs) {
...
export class CBuilder {
    ...
    clone() {
        ...
        let mmEntries = Object.entries(this.memoMaps);
        let mmEntries2 = mmEntries.map(([k, v]) => [k, v.clone()]);
        let mmObj = Object.fromEntries(mmEntries2);
        that.memoMaps = mmObj;
    ...
    pushNewCtx() {
        ...
        let adaptedVars = memoData.mkMemoMap();
    ...
    freshReprType(tag, arity, args, name) {
        let ty;
        if (name === undefined) {
...
function stringToNameHint(maxLen, unprintableChar, value) {
    let nameHint = "";
...
function buildCommonString(cb, strs) {
    strs.forEach(s => {
...
function cCode(code) { return { tag: "CCode", code: code }; }
function cVar(name, depth) { return { tag: "CVar", name: name, depth: depth }; }
function cInt(value) { return { tag: "CInt", value: value }; }
function cStr(value) { return { tag: "CStr", value: value }; }
function cOp(opName, args) { return { tag: "COp", opName: opName, args: getExprs(args) }; }
function cCast(ty, expr) { return { tag: "CCast", ty: ty, expr: getExpr(expr) }; }
function cField(aggregate, field) { return { tag: "CField", aggregate: getExpr(aggregate), field: field }; }
function cList(elems) { return { tag: "CList", elems: getExprs(elems) }; }
function cCall(func, args) { return { tag: "CCall", func: getExpr(func), args: getExprs(args) }; }
function cCommentExpr(comment, expr) { return { tag: "CCommentExpr", comment: comment, expr: getExpr(expr) }; }
function cParens(expr) { return { tag: "CParens", expr: getExpr(expr) }; }
function cAggregate(elems) { return { tag: "CAggregate", elems: getExprs(elems), isConst: false, isVertical: false }; }
function cAggregateVert(elems) { return { tag: "CAggregate", elems: getExprs(elems), isConst: false, isVertical: true }; }
function cAggregateConst(elems) { return { tag: "CAggregate", elems: getExprs(elems), isConst: true, isVertical: false }; }
function cExprStmt(expr) { return { tag: "CExprStmt", expr: getExpr(expr) }; }
function cIf(cond, then) { return { tag: "CIf", cond: getExpr(cond), then: then }; }
function cIfElse(cond, then, els) { return { tag: "CIfElse", cond: getExpr(cond), then: then, else: els }; }
function cWhile(cond, body) { return { tag: "CWhile", cond: getExpr(cond), body: body }; }
function cDoWhile(body, cond) { return { tag: "CDoWhile", body: body, cond: getExpr(cond) }; }
function cReturn(expr) { return { tag: "CReturn", expr: getExpr(expr) }; }
function cCommentStmt(comment) { return { tag: "CCommentStmt", comment: comment }; }
function cBreak() { return { tag: "CBreak" }; }
function cContinue() { return { tag: "CContinue" }; }
function cAssignStmt(lhs, rhs) { return cExprStmt(cOp("=", [lhs, rhs])); }
function cLabel(label) { return { tag: "CLabel", label: label }; }
function cGoto(label) { return { tag: "CGoto", label: label }; }
function cBlock(stmts) { return { tag: "CBlock", stmts: stmts }; }
function cFunc(name, dom, cod, body) { return { tag: "CFunc2", funcVar: getVar(name), dom: dom, cod: cod, body: body }; }
function cVarDecl(ty, varC, defn) { return { tag: "CVarDecl", ty: ty, var: getVar(varC), defn: getExpr(defn) }; }
function cVarDeclUndefined(ty, varC) { return { tag: "CVarDecl", ty: ty, var: getVar(varC) }; }
function cStructDecl(name, fields) { return { tag: "CStructDecl", name: name, fields: fields }; }
function cTypeDefFuncPtr(ty, domRs, codR) { return { tag: "CTypeDefFuncPtr", name: ty, domRs: domRs, codR: codR }; }
function cTypeDef(name, defn) { return { tag: "CTypeDef", name: name, defn: defn }; }
function cExtern(ty, name) { return { tag: "CExtern", ty: ty, name: name }; }
function cAddrOf(expr) {
    return cOp("&_", [expr]);
}
function cDeclVar(varC, defn) {
    let ty = reprToCType(varC.repr);
    let result = cVarDecl(ty, varC, defn);
    return result;
}
function cDeclVarUndefined(varC) {
    let ty = reprToCType(varC.repr);
    let result = cVarDeclUndefined(ty, varC);
    return result;
}
function cDeclConst(varC, defn) {
    let ty = reprToCType(varC.repr);
...
let patMatFail_fatalError = [cExprStmt(cCall(cCode("fatalError"), [cStr("pattern match failure / type-error")]))];
let patMatFail_break = [cBreak()];
function patMatFail_returnNil(cb, repr) {
    let no = reprConvertFromNil(cb, repr);
    return [cReturn(no)];
}
function patMatFail_goto(label) {
    return [cGoto(label)];
}
...
function can_pat_mat(cb, ty, varC2, pat) {
    let failStmts = [];
    let ok;
    cb.pushNewCtx();
    {
...
function cgc_pat_mat(cb, ty, varC, pat, failStmts) {
    function cg(ty, varC, pat) {
        ...
        switch (pat.tag) {
            case "EDatum": {
                ...
                let cond = cOp("!", [cg_eq(cb, varC, cgc_expr(cb, pat))]);
            ...
            case "EList": {
                ...
                switch (varC.repr.tag) {
                    case "Any": {
                        ...
                        let ok = true;
                        pat.exprs.forEach(p => {
                            let cond = cOp("!", [cCall(cCode("any_isPair"), [listVar])]);
                        ...
                        if (pat.tail === null) {
                            if (tyL.tag !== "TNil") {
                                let cond = cOp("!", [cCall(cCode("any_isNil"), [listVar])]);
                                cb.addStmts([cIf(cond, failStmts)]);
                            }
                        }
                        else {
                    ...
                    case "List": {
                        ...
                        let ok = true;
                        pat.exprs.forEach((p, i) => {
                            let cond = cOp("!", [cg_isPair(cb, listVar)]);
                            // if (!(tyL.tag === "TPair" || (tyL.tag === "TAs" && tyL.type.tag === "TPair"))) {
                            //     cb.addStmts([cIf(cond, failStmts)])
                            // }
                            if ((tyL.tag !== "TPair" && (tyL.tag !== "TAs" || tyL.type.tag !== "TPair"))) {
                        ...
                        if (pat.tail === null) {
                            if (tyL.tag !== "TNil") {
                                let cond = cOp("!", [cg_isNil(cb, listVar)]);
                                cb.addStmts([cIf(cond, failStmts)]);
                            }
                        }
                        else {
...
function defaultReprForType(cb, ty, env, targetRepr, nameHint) {
...
function addReprComment(cb, msg, attrs, reprs) {
...
function createFuncRepr(cb, domRs, codR) {
    let memoKey = [domRs, codR];
...
function createFuncRepr_codegen(cb, funcRepr) {
    ...
    cb.addGlobalStmts("AuxC", [cVarDecl(tConst(tName("ReprFunc")), funcRepr.reprC.expr, cAggregateConst([
...
function createClosRepr(cb, domRs, codR, nameHint) {
    let memoKey = [domRs, codR];
    ...
    cb.addGlobalStmts("AuxH", [cStructDecl(cShowType(closTypeV), [["func", funcTypeV],
            ["env", tVoidConstPtr],
        ])]);
...
function createClosRepr_codegen(cb, closRepr) {
    ...
    cb.addGlobalStmts("AuxC", [cVarDecl(tConst(tName("ReprClos")), closRepr.reprC.expr, cAggregateConst([
...
function createFuncNullRepr(cb, domRs, codR) {
    let memoKey = [domRs, codR];
...
function createFuncNullRepr_codegen(cb, closRepr) {
    let domRs = closRepr.dom;
    let codR = closRepr.cod;
    let domReprC = cAggregate(domRs.map(r => reprToReprExpr(r)));
    let codReprC = reprToReprExpr(codR);
    ...
    let funcApply_sd = createFunctionApplySD(cb, hasEnv, domRs, codR);
    let funcApply_ds = cCode("NULL");
    cb.addGlobalStmts("AuxC", [cVarDecl(tConst(tName("ReprFuncNull")), closRepr.reprC.expr, cAggregateConst([
...
function createFunctionApplySD(cb, hasEnv, domR, codR) {
    let memoKey = [hasEnv, domR, codR];
    ...
    let codCTy = reprToCType(codR);
    let envPtrMb = hasEnv ? [tVoidConstPtrConst] : [];
...
function createFunctionApplyDS(cb, hasEnv, domRs, codR) {
    let memoKey = [hasEnv, domRs, codR];
    ...
    let argParams = args.map(a => [a.expr.name, reprToCType(a.repr)]);
...
function cgc_lambda3(cb, expr, nameHint, targetRepr) {
    switch (expr.tag) {
        case "ELambda":
        case "ELambdaYes":
        case "ELambdaNo":
        case "ELambdaMaybe": {
            ...
            let initParams = [];
            let lastLambda = expr;
            let body = lastLambda.body;
            let targetBodyRepr = reprCod(targetRepr);
            ...
            let closEnv = Object.keys(freeVars).map(v => {
                if (v in cb.getEnv()) {
            ...
            cb.pushNewCtx();
            {
                ...
                lastParamMb.forEach(([p, ty]) => {
                    ...
                    let paramVar = cb.freshVar(paramR, rootPatName(p));
                    paramVRs.push(paramVar);
                    let patMatFailStmts = (lastLambda.tag === "ELambdaNo" || lastLambda.tag === "ELambdaMaybe")
            ...
            let domVR = paramVRs.map(vr => [vr.expr.name, reprToCType(vr.repr)]);
            ...
            if (hasEnv) {
                ...
                let closEnvFields = closEnv.map(([name,]) => {
                ...
                let closVars = closEnv.map(([v, vr]) => vr.expr);
                ...
                let closRepr = funcR;
                let closValue = cCast(closRepr.closCTy, cAggregate([
...
function scSkip() { return { tag: "ScSkip" }; }
function scAssign(va) { return { tag: "ScAssign", var: va }; }
function scBreak(sc) { return { tag: "ScBreak", sc: sc }; }
function scContinue(sc) { return { tag: "ScContinue", sc: sc }; }
function scBreakContinue(breakSc, continueSc) { return { tag: "ScBreakContinue", breakSc: breakSc, continueSc: continueSc }; }
// function scGoto(sc: StmtCtx, label: string): StmtCtx { return { tag: "ScGoto", sc: sc, label: label } }
function scMaybe(yesSc, noSc) { return { tag: "ScMaybe", yesSc: yesSc, noSc: noSc }; }
function scGoto(sc, label) {
...
function getTargetReprFromStmtCtx(cb, stmtCtx) {
    let gtr = (sc) => getTargetReprFromStmtCtx(cb, sc);
    switch (stmtCtx.tag) {
        case "ScAssign":
...
function sc_close(cb, stmtCtx, expr) {
    let scc = (sc, er) => sc_close(cb, sc, er);
    switch (stmtCtx.tag) {
        case "ScSkip": {
        ...
        case "ScMaybe": {
            if (expr.repr.tag === "Yes") {
                ...
                cb.addStmts([cIfElse(cg_isNil(cb, rhs2), thenStmts, elseStmts)]);
            }
            return;
        }
        case "ScBreakContinue": {
            ...
            let [elseStmts,] = cb.popCtx();
            cb.addStmts([cIfElse(cg_eq(cb, cg_head(cb, rhs2), cb.staticStrings["break"]), thenStmts, elseStmts)]);
            return;
        }
        case "ScGoto":
            scc(stmtCtx.sc, expr);
...
function stmt_if(cb, funcName, args) {
...
function stmt_ifNil(cb, funcName, args) {
...
function stmt_while(cb, funcName, args) {
...
function stmt_while2(cb, funcName, args) {
...
function stmt_loop2(cb, funcName, args) {
    if (funcName === "loop2" && args.length === 2 && args[1].tag === "ELambda") {
        ...
        return (cb, stmtCtx) => {
            ...
            cb.addStmts([cDeclVarUndefined(breakVar),
...
function stmt_forFoldLeft(cb, funcName, args) {
    if (funcName === "forFoldLeft" && args.length === 3 && args[2].tag == "ELambda" && args[2].body.tag == "ELambda") {
        ...
        return (cb, stmtCtx) => {
            ...
            cb.addStmts([cCommentStmt("stmt_forFoldLeft"),
                cDeclVar(zVar, zValC),
            ...
            cb.pushNewCtx();
            {
                cb.addStmts([cIf(cg_isNil(cb, xVar), [cBreak()]),
...
function stmt_match(cb, funcName, args) {
...
function stmt_guardTrue(cb, funcName, args) {
...
let cgStmt_expr_funcs = [
// stmt_lambda
];
function isStringyRepr(repr) {
    return repr.tag === "Str" || repr.tag === "Single" || repr.tag === "Char";
}
function cg_eq(cb, lhs, rhs) {
...
function expr_length(cb, funcName, args) {
...
function expr_reverse(cb, funcName, args) {
...
function expr_lookup(cb, funcName, args) {
...
//     return closVar
// }
function getFuncArgs_OrNull(cb, expr) {
    let args = [];
    let expr2 = expr;
...
function cgFuncArgs_OrNull(cb, expr) {
...
function find_stmt_func(cb, expr) {
    let args = [];
    let expr2 = expr;
    while (expr2.tag === "EApply") {
        args.unshift(expr2.arg);
...
function ctx_loop_break_continue(cb, stmtCtx, expr) {
...
function tryCtxFunc(cb, stmtCtx, expr) {
...
function cgc_expr_stmt(cb, expr, targetRepr) {
...
function cgc_decl_stmt(cb, defn, pat, env) {
...
function cgc_stmt_apply_func_inline(cb, sc, func, arg) {
    switch (func.tag) {
        case "ELambda": {
            let sc2 = sc;
            let ty = arg.ty1;
...
function cgc_stmt_stmt(cb, expr, stmtCtx) {
    while (expr.tag === "ELet") {
...
function createTupleRepr(cb, elemReprs, nameHint) {
    let memoKey = elemReprs;
    let tupleR = cb.memoMaps.tupleReprMemo.get(memoKey);
    if (tupleR !== undefined) {
        return tupleR;
    }
    ...
    let fields = elemReprs.map((r, i) => [`_${i}`, reprToCType(r)]);
    cb.addGlobalStmts("AuxH", [cStructDecl(cShowType(structName), fields)]);
    let fieldsC = elemReprs.map((r, i) => {
...
function createMaybeRepr(cb, valRepr, nameHint) {
    let memoKey = valRepr;
    let maybeR = cb.memoMaps.maybeReprMemo.get(memoKey);
    if (maybeR !== undefined) {
        return maybeR;
    }
    let [maybeRepr, maybeName] = cb.freshReprType("Maybe", null, [valRepr], nameHint);
    cb.addGlobalStmts("AuxH", [cStructDecl(cShowType(maybeName), [["isYes", tName("bool")],
...
function createYesRepr(cb, elemRepr) {
    let memoKey = elemRepr;
    let yesR = cb.memoMaps.yesReprMemo.get(memoKey);
    if (yesR !== undefined) {
        return yesR;
    }
...
function createListRepr(cb, elemRepr, nameHint) {
    let memoKey = elemRepr;
    let listTy = cb.memoMaps.listReprMemo.get(memoKey);
    if (listTy !== undefined) {
        return listTy;
    }
    ...
    let listTy2 = rList(listCType, listReprExpr2, elemReprExpr, elemRepr);
...
//     return listTy2
// }
function createUnionRepr(cb, altReprs, nameHint, useMemoMap = true) {
    let memoKey = altReprs;
    if (useMemoMap) {
        let unionR = cb.memoMaps.unionReprMemo.get(memoKey);
        if (unionR !== undefined) {
            return unionR;
        }
    }
    ...
    let fields = [["tag", tName("int")], ["value", tUnion(unionFields)]];
    ...
    cb.addGlobalStmts("AuxC", [cVarDecl(tConst(tName("ReprUnion")), unionReprExpr, unionReprAgg)]);
    let listTy2 = rUnion(unionCType, unionReprExpr.expr, altReprs);
    if (useMemoMap) {
        cb.memoMaps.unionReprMemo.set(memoKey, listTy2);
    }
    return listTy2;
...
//     return inR
// }
function createPtrRepr_init(cb, ty, nameHint) {
...
function createPtrRepr_complete(cb, ptrRepr, destRepr) {
...
function createRecursiveRepr(cb, tyRec, nameHint) {
    tyRec = knownInhabited(tyRec);
    let memoKey = tyRec;
    let recR = cb.memoMaps.recReprMemo.get(memoKey);
    if (recR !== undefined) {
        return recR;
    }
...
function createSingleRepr(cb, value) {
    let memoKey = value;
    let singleR = cb.memoMaps.singleReprMemo.get(memoKey);
    ...
    let listTy2 = rSingle(singleCType, singleReprExpr.expr, value);
    cb.memoMaps.singleReprMemo.set(memoKey, listTy2);
    return listTy2;
}
function reprTupleProjection(cb, repr, pos) {
    if (repr === undefined) {
        return undefined;
    }
    switch (repr.tag) {
...
function reprConvertFromNil(cb, repr) {
    switch (repr.tag) {
        case "Any": {
            let nil = anyExpr(cCall(cCode("any_nil"), []));
            return nil;
        }
        case "List": {
...
function cgc_tuple(cb, elems, targetRepr) {
    let elemReprs = [];
    let elemsC = [];
    elems.forEach((e, pos) => {
...
function cgc_list2(cb, elems, targetRepr) {
    if (elems.tag !== "EList") {
    ...
    let listRepr = createListRepr(cb, reprListElem(listR));
    ...
    cb.addStmts([
        cAssignStmt(cField(listVar, "elems"), cCall(cCode("list_prependN"), [reprToReprExpr(listRepr.elemRepr),
...
function cgc_call(cb, func, args) {
    let funcC = func;
    let funcR = func.repr;
...
function cgc_expr(cb, expr, targetRepr) {
    if (USE_MIXINS) {
...
function cgc_expr_base(cb, expr, targetRepr) {
    let cg = (expr, targetRepr) => cgc_expr(cb, expr, targetRepr);
    switch (expr.tag) {
        case "EVar":
        ...
        case "EList": {
            ...
            if (expr.exprs.length !== 0 && (expr.ty1.tag === "TList" || expr.ty2?.tag === "TList" || targetRepr?.tag === "List")) {
                ...
                {
                    let list = cgc_list2(cb, expr, targetRepr);
                    return list;
                }
                // else {
            ...
            if (expr.tail === null && expr.exprs.length !== 0) {
                let tuple = cgc_tuple(cb, expr.exprs, targetRepr);
                return tuple;
            }
...
//     cgc_pat_mat(cb, pat.ty1, varC, pat, patMatFail_fatalError)
// }
function cgc_decl_local(cb, decl, env) {
    let [pat, defn] = decl;
...
function cgc_decls(cb, decls, env) {
    decls.forEach(d => {
        cgc_decl(cb, d, env);
    });
}
function cQuoteStr(q, s) {
    let chars = [];
    let len = s.length;
    for (let i = 0; i !== len; i++) {
        let c = s.charAt(i);
...
function cShowStmt(out, indent, stmt) {
    let indent2 = indent + "    ";
    switch (stmt.tag) {
        case "CReturn":
...
function cShowStmts(out, indent, stmts) {
    stmts.flatMap(s => cShowStmt(out, indent, s));
}
function cShowDecl(out, indent, decl) {
    switch (decl.tag) {
        case "CFunc2": {
...
export function codegen_c_primitive(cb) {
    const s = [];
    ...
    const primFunc = cFunc(primName, [["name", tStr]], tAny, s);
    cb.addGlobalStmts("AuxC", [primFunc]);
}
export function codegen_c_expr(cb, expr) {
    let exprStr;
    cb.pushNewCtx();
    {
...
export function codegen_c_stmts(cb, decls, env) {
    for (const decl of decls) {
        cgc_decl(cb, decl, env);
    }
}
export function codegen_reprs(cb) {
...
//# sourceMappingURL=codegen-c.js.map