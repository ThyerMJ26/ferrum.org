...
import { projectClone, emptyProject, readProject } from "../syntax/project.js";
import { scan2Fe } from "../syntax/scan.js";
import { showLoc, nilLoc } from "../syntax/token.js";
import { Output, visitAll, showExp2, showExprTy1, showExprTy2 } from "../syntax/expr.js";
// Tree
import { collectTypeErrors, showType4, showTypeDiff, showType2, countTypeErrors, showTypeWithoutSubSuperBounds, nodeToType } from "../tree/types.js";
import { tcSummariseGraph2, tcSummariseTree2 } from "../graph/code-table.js";
import { depthZero } from "../graph/graph-heap2.js";
...
export function truncateStr(maxLen, value) {
...
let projectCache = {};
export async function loadProjectForTestDefn2(td, testDir, initCt) {
    const io = getIo();
    ...
    if (cachedProj !== undefined) {
        ...
        const result = [proj, projCt];
        projectCache[td_project] = result;
        return result;
    }
}
function collectAndShowTypeErrors(decls) {
...
function printTypeDiffReport(decls) {
    let call = (field, exp) => {
...
export function someTrueNoneFalse(...elems) {
    let someTrue = false;
    let someFalse = false;
...
export async function startTest(initCt, td, opts, codeRunnerName, testDir, useLineStarts = true) {
    const io = getIo();
    const co = {
        tyT: true,
    ...
    for (const check of td.checks) {
        ...
        const trp = { partName: checkName, check };
...
export function continueTest(tr) {
    if (tr.finished)
        return true;
    let projCt = tr.ct;
    const opts = tr.opts;
    ...
    try {
        ...
        if (doBidir && opts.showTypes) {
            ...
            if (opts.useGraphTypes) {
                ...
                console.log(showExp2(testCt.declsTy(), "Expr: Syn", testCt.graph().show.showExprTySyn));
                console.log(showExp2(testCt.declsTy(), "Expr: Ctx", testCt.graph().show.showExprTyCtx));
                console.log("");
            }
        }
...
export async function runTest(initCt, td, opts, codeRunnerName, testDir, useLineStarts = true) {
...
//#endregion
export function showTestResultMnemonic(tr, ref) {
...
export function testResultMnemonics(tr) {
    return [
        ["TyCT", tr.typeCheckT],
        ["TyCG", tr.typeCheckG],
        ["TyMT", tr.typeMatchT],
