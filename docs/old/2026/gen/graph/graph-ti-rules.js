...
import { depthInc, false0, isAddrYes, addrNo, depthZero, addrTypeType, assumeIsType, } from "../graph/graph-heap2.js";
import { tiFalse, tiTrue, tiUnknown, tieFalse, tieTrue, tieUnknown } from "../graph/graph-ti.js";
import { mkSubstitute } from "./graph-substitute.js";
export function mkTiRules(h, tim, p, tis) {
    const { directAddrOf, depthOf, tyPrim0, tyPrim1, tyPrim2, isTyPrim, isTyOp, isTyPrimOneOf, isTyPrimOneOf2, isTyVoid, isTyNil, isTyList, isTyAny, name_ty, arg0_ty, arg1_ty, isTySingleStr, value_ty, tyPair, isTyPair, hd_ty, tl_ty, tyFun, isTyFun, no_ty, yes_ty, dom_ty, cod_ty, tyVar, isTyVar, tmVar, tmLam, isTmLam, pat_tm, body_tm, tyApply, isTyApply, fun_ty, arg_ty, tmApply, tyOp1, tyOp2, tyHead, tyTail, tyDom, tyCod, } = h;
    const tyRelComp /**/ = (a, b, depth) => h.tyCon2("{\\}", a, b, depth);
    const tyUnion /**/ = (a, b, depth) => h.tyCon2("{|}", a, b, depth);
    const tyIntersect /**/ = (a, b, depth) => h.tyCon2("{&}", a, b, depth);
    const dOf = h.directAddrOf;
    const tyElem = (arg, depth) => h.tyPrim1("Elem", arg, depth);
    const typeType = addrTypeType;
    const tyPairAnyAny = tyPair(tyPrim0("Any", depthZero), tyPrim0("Any", depthZero), depthZero);
    function tySub(a, b, depth) {
        return tyOp2("{<:}", a, b, depth);
    }
    const { tieConst, tieRef, tieAnd, tieAndImp, tieOr, tieOrImp, tieNot, 
    // tieFalse,
    // tieTrue,
    // tieUnknown,
    tieAndRefs, tieAndImpRefs, tieOrRefs, tieOrImpRefs, tieAndUnknown, tieOrUnknown, timKey, } = tim;
    const subst = mkSubstitute(h);
    function substTy(appDepth, varDepth, arg, body) {
        // const env: SubstEnv = new Map
        const env = null;
        let result = subst.substTmTy(appDepth, varDepth, env, arg, body);
        return result;
    }
    let tiRules = [
        // ["Compute", computeRule],
    ...
    function relCompRule(ty0) {
        ...
        if (isTyOp("{\\}", ty)) {
            ...
            const disjointTyOps = ["Int", "Bool", "Type", "Str", "Char", "Nil"];
            ...
            const disjointTypes = (a) => isTySingleStr(a) || isTyFun(a) || isTyPrimOneOf(disjointTyOps, a);
    ...
    // { A \ { Any \ B1 \ ... \ Bn } } -> { A & { B1 | ... | Bn } }
    function rcAnyRelCompBRule(ty0) {
        const ty = dOf(ty0);
        const depth = depthOf(ty);
        if (isTyOp("{\\}", ty)) {
            const a = arg0_ty(ty);
    ...
    // { A.single \ { B0 | B1 } }
    function rc_SingleA_UnionB_Rule(ty0) {
        const ty = dOf(ty0);
        const depth = depthOf(ty);
        if (isTyOp("{\\}", ty)) {
            const a = dOf(arg0_ty(ty));
    ...
    function rc_NonUnionA_UnionB_Rule(ty0) {
        assert.todo();
    }
    // { A \ { B  | { C   \ D   } } } -> { { A | D   } \ { B  \ C   } }
    // { A \ { B0 | { B10 \ B11 } } } -> { { A | B11 } \ { B0 \ B10 } }
    function rcUnionRelCompBRule(ty0) {
        const ty = dOf(ty0);
        const depth = depthOf(ty);
        if (isTyOp("{\\}", ty)) {
            const a = dOf(arg0_ty(ty));
    ...
    // { (Codomain { A0 & A1 }) \ B }
    function rcCodomainIntersectARule(ty0) {
        const ty = dOf(ty0);
        const depth = depthOf(ty);
        if (isTyOp("{\\}", ty)) {
            const a = dOf(arg0_ty(ty));
    ...
    function maxDepth2(a, b) {
        return Math.max(depthOf(a), depthOf(b));
    }
    // { (List A.elem) \ (Rec B.body) }
    function rcListRecRule(ty0) {
        const ty = dOf(ty0);
        // const depth = depthOf(ty)
        if (isTyOp("{\\}", ty)) {
            const a = dOf(arg0_ty(ty));
    ...
    // { (Elem (Tail A)) \ (Elem B) }
    function rcElemTailElemRule(ty) {
        const depth = depthOf(ty);
        if (isTyOp("{\\}", ty)) {
            const a = dOf(arg0_ty(ty));
            const b = dOf(arg1_ty(ty));
    ...
    function tryUnrollRecursiveType(ty) {
        if (!(isTyPrim("Fix", ty)))
            return addrNo;
        const lam = dOf(arg0_ty(ty));
        if (!(isTmLam(lam)))
    ...
    function rcFuncRule(ty) {
        ...
        if (isTyOp("{\\}", ty)) {
            ...
            if (isTyFun(a) && isTyFun(b)) {
                ...
                const aSubstEnv = new Map;
                aSubstEnv.set(h.pathKey_root, tmVar([], varDepth, polyDomB));
                const bCod = dOf(cod_ty(b));
                const bSubstEnv = new Map;
                bSubstEnv.set(h.pathKey_root, tmVar([], varDepth, polyDomB));
                const appA = subst.substTmTy(varDepth, aVarDepth, aSubstEnv, polyDomB, aCod);
                const appB = subst.substTmTy(varDepth, bVarDepth, bSubstEnv, polyDomB, bCod);
    ...
    function applyTypes(fun, argTy) {
        return tyApply(fun, argTy);
    }
    // { A \ { B.dom -> B.rng } }
    function rcFuncBRule(ty) {
        const depth = depthOf(ty);
        if (isTyOp("{\\}", ty)) {
            const a = dOf(arg0_ty(ty));
            const b = dOf(arg1_ty(ty));
    ...
    function rcFuncIntersectBRule(ty) {
        const depth = depthOf(ty);
        if (isTyOp("{\\}", ty)) {
            const a = dOf(arg0_ty(ty));
            const b = dOf(arg1_ty(ty));
    ...
    function rcApplyIntersectyRule(ty) {
        const depth = depthOf(ty);
        if (isTyOp("{\\}", ty)) {
            const a = dOf(arg0_ty(ty));
            const b = dOf(arg1_ty(ty));
        ...
        if (isTyOp("{\\}", ty)) {
            ...
            if (isTyPrim("Self", a)) {
                ...
                if (isTmLam(a0)) {
                    ...
                    let a1 = subst.substTmTy(depthOf(a), depthVar, env, addrTypeType, a0body);
...
//# sourceMappingURL=graph-ti-rules.js.map