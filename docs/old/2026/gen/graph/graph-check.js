import { depthInc, isAddrYes } from "../graph/graph-heap2.js";
function reportError(errMsg) {
    console.error(errMsg);
    // throw new Error(errMsg)
}
export function check(h, rootAddr) {
    const rootDepth = h.depthAt(rootAddr);
    let ok = true;
    const done = new Set;
    const todo = [[null, rootAddr, rootDepth]];
    while (todo.length !== 0) {
...
//# sourceMappingURL=graph-check.js.map