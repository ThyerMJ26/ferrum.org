...
import { addrTypeType, depthMax, depthZero } from "../graph/graph-heap2.js";
import { tiIsTrue, tiIsUnknown, tiNot, tiIsKnown, tiIsFalse, tiFalse, tiTrue, tiUnknown, tiContradiction, tiIsContradiction, tiInternalError, tieInternalError } from "../graph/graph-ti.js";
import { graphShowExpr } from "../graph/graph-show.js";
import { showLoc } from "../syntax/token.js";
import { FuelExhausted } from "../ui/fuel.js";
export const mkTiCalcFuncs = (h, gr, timf, tis, tiRules, monitor) => {
    const { depthOf, directAddrOf, } = h;
    const tim = timf.tim;
    ...
    const showExpr = (addr) => graphShowExpr(h, addr);
    const timKey = (ty) => {
        const key = ty;
        if (!tim.has(key)) {
    ...
    function typeCheck(torp, synTy, ctxTy, loc) {
    ...
    function typeInhabCheck(rc, loc) {
        const key1 = timKey(rc);
        const entry = tim.get(key1);
        return entry.value;
    }
    function tiEvalExpr(tie) {
    ...
    function tiMergeValues(vals) {
        let result = tiUnknown;
        let anyNulls = false;
        const causeKnown = [];
        const causeUnknown = [];
        const causeContradiction = [];
        vals.forEach((val, i) => {
            if (val !== null) {
    ...
    function tiNext(tyKey) {
        const resultExpand = [];
        const resultReduce = [];
        const visited = {};
        const todo = [[tyKey, null]];
        while (todo.length !== 0) {
        ...
        try {
            ...
            calcLoop: while (entry.value === null) {
                ...
                for (const key of toReduce) {
                    ...
                    const expr_values = entry.exprs.map(([ruleName, exp]) => {
                        let result = [ruleName, exp, tiEvalExpr(exp)];
    ...
    function showTiExpr(tie) {
        let stie = (tie) => showTiExpr(tie);
        switch (tie.tag) {
            case "TiConst":
    ...
    function tiSummaryGraph(ty) {
        const summaryGraph = new Map();
        const todo = [];
        const key1 = timKey(ty);
        todo.push(key1);
...
//# sourceMappingURL=graph-ti-calc.js.map