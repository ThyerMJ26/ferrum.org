...
import { showLoc, locMerge } from "../syntax/token.js";
import { eLambda, eLambdaMaybe, eLambdaNo, eLambdaYes, eAs, eApply, eVar, ePrim, eTypeAnnot, eTermBrackets, eTypeBrackets, exprTransform, exprTransform2PairList } from "../syntax/expr.js";
import { operatorParser, mkOperatorTable } from "./operator.js";
// TODO Move these into the ParseState interface.
function parseToken(ps, tag, val, seeAlsoToken) {
    const tok = ps.take();
...
function tryParseToken(ps, tag, val) {
    const tok = ps.peek();
    if (tok.tag !== tag || tok.value !== val) {
        return false;
    }
...
export function mkOpInfix(tort, opDefn, loc, arg1, arg2) {
    let show = (a) => JSON.stringify(a);
    if (arg1 === undefined) {
...
function mkOpPrefix(tort, opDefn, loc, arg1) {
    switch (opDefn.nameC) {
        case "->": {
            // let unitArg: Expr = { tag: "EDatum", value: null, loc: loc }
            let unitArg = { tag: "EList", exprs: [], tail: null, loc: loc };
...
function mkOpPostfix(tort, opDefn, loc, arg1) {
    switch (opDefn.nameC) {
        default: {
            const nameP = opDefn.nameP[tort];
            assert.isDefined(nameP);
...
const mkOp = {
    mkApply(op, func, arg) {
        ...
        const arg2 = { tag: "EApply", loc: loc, func, arg, op: "" };
        return arg2;
    },
...
function mkFerrumOperatorTable() {
    const ot = mkOperatorTable();
    /*** Rules ***/
    ot.opRule("As", "None", []);
...
function parseExpr(ps, seeAlsoToken = null) {
    const allowLet = true;
...
function tryParseExprPart(ps, allowLet) {
    if (ps.eof()) {
        return null;
    }
    let tok = ps.peek();
    switch (tok.tag) {
        ...
        case "ident": {
            ps.take();
            let expr1 = { tag: "EVar", name: tok.value, loc: tok.loc };
            return expr1;
        }
        case "keyword": {
            switch (tok.value) {
        ...
        case "symbol": {
            ps.take();
            let expr1 = { tag: "ESym", name: tok.value, loc: tok.loc };
            return expr1;
        }
        case "eof":
            return null;
...
export function parseFile(ps, language) {
    switch (language) {
        case "Ferrum/0.1":
        case "ferrum/0.1": {
...
function parseExp(ps, language, tort) {
    switch (language) {
        case "Ferrum/0.1":
        case "ferrum/0.1":
...
export const parseTerm = (ps, language) => {
    const result = parseExp(ps, language, "Term");
    return result;
};
export const parseType = (ps, language) => parseExp(ps, language, "Type");
export function stripNonSemanticBrackets_expr(expr, tort = "Term") {
    // return expr
    switch (tort) {
        case "Term":
...
export function stripNonSemanticBrackets_decls(decls) {
    return exprTransform2PairList(decls, { expr: stripNonSemanticBrackets_expr });
}
//# sourceMappingURL=parseFerrum2.js.map