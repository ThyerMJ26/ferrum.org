...
export let emptyProject = {
    filename: "",
    fileContents: "",
    parts: [],
    contents: new Map,
};
export function projectClone(p) {
    return {
        filename: p.filename,
...
export async function readProject(projectFilename) {
    const io = getIo();
    // assert.isTrue(io.path_resolve(projectFilename) === projectFilename)
    let proj = projectClone(emptyProject);
    proj.fileContents = await io.vfs_read(projectFilename);
    let p = FDN_parse(proj.fileContents);
