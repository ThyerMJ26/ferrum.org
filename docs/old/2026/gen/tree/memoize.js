...
class MemoDataImpl {
    ...
    dataMemoIn = new Map();
    dataMemoOut = new Map();
    arrayMemo = new MemoTree();
    objectMemo = new MemoTree();
    funcMemo = new Map();
    funcStats = {};
    ...
    memoizeFunction(funcName, func) {
        ...
        let func2 = (...args) => {
            ...
            let result = this.idToData(resultId);
            return result;
        };
    ...
    loadLine(line) {
        ...
        switch (items[0]) {
            ...
            case "array": {
                ...
                let arrayVal = keys.map((a) => this.idToData(a));
            ...
            case "object": {
                ...
                let names = this.idToData(namesArrayId);
                let values = valueIds.map((id) => this.idToData(id));
    ...
    //     return id
    // }
    addNewData(tag, key, val) {
        let id = this.genId();
        this.dataMemoIn.set(val, id);
        this.dataMemoOut.set(id, val);
        let logLine = [tag, id, key];
