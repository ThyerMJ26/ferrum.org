import { showPos, showLoc, mkPos, mkToken, posCopy } from "../syntax/token.js";
import { logger_log } from "../utils/logger.js";
function isElem(item, candidates) {
...
export function isAlphaUS(ch) {
    return isAlpha(ch) || ch === "_";
}
function isAlphanumUSP(ch) {
    return isAlphanum(ch) || ch === "_" || ch === "'";
}
export function isAlphanumUS(ch) {
...
export function scan(filename, input, offsetPos = null, language = null) {
    switch (language) {
        // case null:
        // case "":
...
export function scanCompare(filename, input, offsetPos = null) {
...
export function scan1(filename, input, offsetPos = null) {
    let result = [];
    // let pos = 0
    // let line = 1
    // let col = 1
    ...
    function output(tag, value) {
        ...
        let tok = mkToken(tag, value, filename, startPos2, pos2, []);
        result.push(tok);
    }
    while (pos.pos < input.length) {
        ...
        if (ch === " ") {
            ...
            let tag = isElem(word, keywords) ? "keyword" : "ident";
            output(tag, word);
        }
        else if (isNum(ch)) {
            let num = 0;
            ...
            let tag = isElem(sym, keysyms) ? "keysym" : "symbol";
            output(tag, sym);
        }
...
export function scan2(filename, input, offsetPos = null) {
    let toks = scan2a(filename, input, offsetPos, keywords, keysyms, []);
    return toks;
}
export function scan2Fe(filename, input, offsetPos = null, lineStarts = []) {
    ...
    let toks2 = toks.map(tok => {
        ...
        if (tok.tag === "ident" && tok.value === "if") {
            let tok2 = { ...tok, tag: "ident", value: "if2" };
            return tok2;
        }
        return tok;
    });
...
export function scan2a(filename, input, offsetPos, keywords, keysyms, inputLineStarts) {
    let result = [];
    ...
    let startPos = posCopy(pos);
    function output(tag, value, tokenLineStarts) {
        logger_log("scan", 1, "Token", pos.line, pos.col, tag, value);
        ...
        let tok = mkToken(tag, value, filename, startPos2, pos2, tokenLineStarts);
        result.push(tok);
    }
    function advancePos(num) {
        for (let i = 0; i !== num; i++) {
            if (pos.pos === input.length) {
                break;
            }
    ...
    function tryScanLiteral(str, then) {
    ...
    function optScanLiteral(str, then) {
...
function scanFileStart(filename, input, offsetPos = null, lineStarts) {
    let pos = 0;
    let line = 1;
    let col = 1;
    let input2 = input;
...
export function scanFile(filename, input, offsetPos = null, defaultLanguage = null, lineStarts) {
