import { lambdaOp } from "../syntax/expr.js";
import { isAlpha } from "../syntax/scan.js";
import { assert } from "../utils/assert.js";
import { uiText, uiTextA, uiTextLength, uiTextList, uiTextToStr } from "../ui/text.js";
export const prettyFerrum = (expr) => {
    ...
    switch (expr.tag) {
        ...
        case "EPair": {
            ...
            const list = { tag: "EList", exprs: elems, tail: tail };
            return p(list);
        }
        case "ELet": {
            ...
            const decls = expr.decls.map(([pat, defn]) => `${p(pat)}=${p(defn)};`);
...
export const pShow = (s, doc) => {
    // const [txt, col] = p2Show(s, doc, "H", 0)
    // return txt
    return p2Show2(s, doc, new Map);
};
const indentZero = 0;
function indentInc(indent, levels = 1) {
    return indent + (levels * pFeIndentIncrement);
}
export const prettyFerrumStyleDefns = {
    std: { bg: "White" },
    // selected: { weight: 1 },
    selected: { bg: "Blue" },
    // selected: { bg: "Red" },
...
//         ];
//     [ [indent, [text2]] ];
function p3LeadingText(indent, text) {
    assert.todo();
}
// const pNil: Doc<never> = []
// const pNil<A>: Doc<A> = []
function p1IsMultiLine(doc) {
    return doc.length > 1;
}
function p2IsMultiLine(doc) {
    switch (doc.tag) {
        case "V":
...
function pVA(annot, ...docs) {
    return [].concat(...docs);
}
function p2VA(annot, ...docs) {
...
function p1FitsOnOneLine(docs, punctLen = 0) {
    let lineLen = 0;
    for (const doc of docs) {
        if (doc.length === 0) {
            continue;
...
function p2FitsOnOneLine(docs, punctLen = 0) {
    let lineLen = 0;
    for (const doc of docs) {
        if (doc.isMultiLine) {
            return false;
...
function pList(indent, style, [opn, sep, cls], docs) {
    const pT = (text) => pText(indent, style, text);
    if (docs.length === 0) {
...
function p2List(indent, style, [opn, sep, cls], docs) {
    const pT = (text) => pTextN(indent, style, text);
    if (docs.length === 0) {
        return pH(pT(opn), pT(cls));
    }
    const first = pH(pT(opn), docs[0]);
...
//         ]
//     ];
function pList2(indent, [opn, sep, cls], a) {
    assert.todo();
}
function pFeList(indent, style, elems, tail) {
    const pT = (text) => pText(indent, style, text);
...
function pLabelList2(elems, tail, build) {
    assert.todo();
}
function pfParensNeeded(ctx, expr) {
...
function pfParensNeeded2(ctx, expr) {
    if (ctx !== null && (ctx[0] === "ETermBrackets" || ctx[0] === "ETypeBrackets")) {
        return false;
    }
    switch (expr.tag) {
...
export function mkPrettyFerrum(maxIndent = 40, maxWidth = 40, styles, selected) {
    let nextId = 0;
    return {
        pExpr: (expr) => pExpr(indentZero, styles, "Term", null, expr),
        pDecls: (decls) => pfDecls0(indentZero, styles, "Term", decls)
    };
    ...
    function pfDecls0(indent, styles, tort, decls) {
        const pT = (text) => pText(indent, styles.std, text);
        const declsD = decls.map(decl => {
            const indent2 = indentInc(indent);
            const pT = (text) => pText(indent2, styles.std, text);
            const pE = (expr) => pExpr0(indent2, styles, tort, null, expr);
            const [pat, defn] = decl;
            const patDD = pE(pat);
            const defnDD = pE(defn);
            return [patDD, defnDD];
        });
        const declsDoc = pV(...declsD.map(([pat, defn]) => pHV(pH(pT("let"), pat.doc, pT("=")), pH(defn.doc, pT(";")))));
        return [declsD, declsDoc];
    }
    function pExpr0(indent, styles, tort, ctx, expr) {
    ...
    function pExpr(indent, styles, tort, ctx, expr) {
        const exprAddr = expr;
        ...
        const pT = (text) => pText(indent, style, text);
        const pTA = (annot, text) => pTextA(indent, style, text, annot);
        const pE0 = (ctx, expr) => pExpr0(indent, styles, tort, ctx, expr);
        const pE1 = (ctx, expr) => pExpr0(indent1, styles, tort, ctx, expr);
        switch (expr.tag) {
            case "EVar": {
                const id = genId();
            ...
            case "EPair": {
                ...
                if (list.tag === "EDatum" && list.value === null) {
                    ...
                    const tlAddr = list.addr;
                    if (tlAddr !== undefined && selected.has(tlAddr)) {
                        tlStyle = styles.selected;
                    }
                    tl = { ...list, id: genId(), doc: pText(indent, tlStyle, "]") };
                ...
                if (pairElems.length === 0) {
                    ...
                    while (pairElem = pairElems.pop()) {
                        ...
                        let doc = pHV(elemED.doc, pText(indent, pairStyle, ","), listED.doc);
                        if (pairElems.length === 0) {
...
// TODO ? Take an empty Id<->Expr Map, to be populated as the ExprDoc is generated
export function mkPrettyFerrum2(maxIndent = 40, maxWidth = 40, styles, idExprMap, exprIdMap, addrIdsMap) {
    let nextId = 0;
    return {
        pExpr: (expr) => pExpr(indentZero, "Term", null, expr),
        pDecls: (decls) => pfDecls0(indentZero, "Term", decls)
    };
    function genId() {
        return nextId++;
    }
    function buildRight_ExprDoc(indent, expr, opn, sep, cls, dn, up) {
        ...
        for (let i = 0; i !== eds.length; i++) {
            ...
            mapListPush(addrIdsMap, e.addr, id);
        }
        return exprDoc;
    }
    function buildLeft_ExprDoc(indent, expr, opn, sep, cls, dn, up) {
        const style = styles.std; // TODO remove styles
        const eds = [];
        let expr2 = expr;
        let [e, ed] = dn(expr2);
    ...
    function pfDecls0(indent, tort, decls) {
        // const pT = (text: string) => pText(indent, styles.std, text)
        const pT = (text) => pTextN(indent, styles.std, text);
        const declsD = decls.map(decl => {
            const indent2 = indentInc(indent);
            const pT = (text) => pTextN(indent2, styles.std, text);
            const pE = (expr) => pExpr0(indent2, tort, null, expr);
            const [pat, defn] = decl;
            const patDD = pE(pat);
            const defnDD = pE(defn);
            return [patDD, defnDD];
    ...
    function pExpr0(indent, tort, ctx, expr) {
    ...
    function pExpr(indent, tort, ctx, expr) {
        const exprAddr = expr;
        const id = genId();
        if (exprAddr.addr !== undefined) {
        ...
        const style = styles.std;
        const pT = (text) => pTextN(indent, style, text);
        const pTA = (annot, text) => p2TextAN(indent, style, text, annot);
        const pE0 = (ctx, expr) => pExpr0(indent, tort, ctx, expr);
        const pE1 = (ctx, expr) => pExpr0(indent1, tort, ctx, expr);
        switch (expr.tag) {
            case "EVar": {
                return { ...expr, id, doc: pTA(id, expr.name) };
            }
            ...
            // }
            case "EApply": {
                const ed = buildLeft_ExprDoc(indent, expr, tortOpen(tort), " ", tortClose(tort), (e) => {
                    return (e.tag === "EApply"
            ...
            case "ELambda": {
                const ed = buildRight_ExprDoc(indent, expr, tortOpen(tort), "->", tortClose(tort), (e) => {
            ...
            case "EPair": {
                ...
                let tl;
                let lastPairED;
                const isRegularList = (tort === "Term" && list.tag === "EDatum" && list.value === null) ||
                ...
                if (isRegularList) {
                    ...
                    switch (list.tag) {
                        case "EDatum":
                            tl = { ...list, id: tlId, doc: p2TextAN(indent, tlStyle, "]", tlId) };
                            break;
                        case "EPrim":
                            tl = { ...list, args: [], id: tlId, doc: p2TextAN(indent, tlStyle, "]", tlId) };
                            break;
                        default:
                            assert.unreachable();
                    }
                    ...
                    mapListPush(addrIdsMap, lastPair.addr, lastPairId);
                    mapListPush(addrIdsMap, tl.addr, tlId);
                }
                else {
                    const lastPairId = genId();
                    tl = pE1([expr.tag, 1], expr.tl);
                ...
                if (pairElems.length === 0) {
                    ...
                    while (pairElem = pairElems.pop()) {
                        const pairId = genId();
                        const [pairE, elemED, pairStyle] = pairElem;
                        let doc = pHV(elemED.doc, pTextN(indent, pairStyle, ","), listED.doc);
                        if (pairElems.length === 0) {
                            doc = pHV(pTextN(indent, pairStyle, "["), doc);
                        }
                        doc = pA(pairId, doc);
                        listED = { ...pairE, id: pairId, hd: elemED, tl: listED, doc };
                        mapListPush(addrIdsMap, pairE.addr, pairId);
                    }
                }
                return listED;
            }
...
//     }
// }
export function p2Show2(s, doc, idTextMap) {
    let col = 0;
    return pShow(doc);
    function pShow(doc) {
        let result;
...
export function prettyFerrum1(expr) {
    // TODO Better handling of text styles.
    const styleZero = 0;
    const styles = { std: styleZero, selected: styleZero };
...
export function prettyFerrum2(expr) {
    // TODO Better handling of text styles.
    const styleZero = 0;
    const styles = { std: styleZero, selected: styleZero };
...
//# sourceMappingURL=pretty-ferrum.js.map