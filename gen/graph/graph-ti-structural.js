import { addrTypeType, depthZero, assumeIsType, assumeIsDirect } from "../graph/graph-heap2.js";
import { tiFalse, tiTrue, tiUnknown, tiAnd, tiAndImp, tiNot, tiOr, tiOrImp, tiIsTrue } from "../graph/graph-ti.js";
export function mkTiStructuralFuncs(h, p) {
    ...
    function dOf(a) { assumeIsDirect(a); return a; }
    const { depthOf, tyPrim0, isTyPrim, isTyOp, isTyPrimOneOf, isTyPrimOneOf2, isTyVoid, isTyNil, isTyList, isTyAny, name_ty, arg0_ty, arg1_ty, isTySingleStr, value_ty, tyPair, isTyPair, hd_ty, tl_ty, tyFun, isTyFun, no_ty, yes_ty, dom_ty, cod_ty, tyVar, isTyVar, tmVar, tmLam, isTmLam, pat_tm, body_tm, tyApply, isTyApply, fun_ty, arg_ty, tyOp1, tyOp2, tyUnion, tyIntersect, tyRelComp, tyHead, tyTail, tyDom, tyCod, } = h;
    ...
    function isPairType(a) {
        const rc = tiStructuralRelComp(defaultStackLimit, a, tyPairAnyAny);
        return tiIsTrue(rc);
    }
    function tiStructural(sl, ty0) {
        if (sl === 0)
            return tiUnknown;
        const ty = dOf(ty0);
        const tis = (ty) => tiStructural(sl - 1, ty);
        const tisRc = (a, b) => tiStructuralRelComp(sl - 1, a, b);
        const tisIn = (a, b) => tiStructuralIntersect(sl - 1, a, b);
        if (isTySingleStr(ty)) {
            return tiTrue;
        }
    ...
    function tiStructuralUnion(sl, aTy, bTy) {
        if (sl === 0)
            return tiUnknown;
        let tis = (ty) => tiStructural(sl - 1, ty);
        return tiOr(tis(aTy), tis(bTy));
    }
    function tiStructuralRelComp(sl, a0, b0) {
        if (sl === 0)
            return tiUnknown;
        let tis = (ty) => tiStructural(sl - 1, ty);
        let tisRc = (a, b) => tiStructuralRelComp(sl - 1, a, b);
        let tisIn = (a, b) => tiStructuralIntersect(sl - 1, a, b);
        const a = dOf(a0);
        const b = dOf(b0);
        if (equalType(a, b)) {
            return tiFalse;
        ...
        const isDisjoint = (a) => isPrims(["Int", "Bool", "Type", "Str", "Char", "Nil"], a) ||
            isTySingleStr(a);
        ...
        const isNonPair = (a) => isPrims(["Any", "Bool", "Type", "Str", "Char", "Nil"], a) ||
    ...
    function tiStructuralIntersect(sl, a0, b0) {
        if (sl === 0)
            return tiUnknown;
        const a = dOf(a0);
        const b = dOf(b0);
        ...
        // We can count SingleStr and Str as disjoint here, as they will be if we reach this far.
        const isDisjoint = (a) => isTySingleStr(a) ||
    ...
    function tiStructuralIntersectAssumeArgsInhabited(sl, a0, b0) {
        if (sl === 0)
            return tiUnknown;
        const a = dOf(a0);
        const b = dOf(b0);
        let tis = (ty) => tiStructural(sl - 1, ty);
        let tisRc = (a, b) => tiStructuralRelComp(sl - 1, a, b);
        let tisInAai = (a, b) => tiStructuralIntersectAssumeArgsInhabited(sl - 1, a, b);
        if (equalType(a, b)) {
            return tiTrue;
        }
...
//# sourceMappingURL=graph-ti-structural.js.map