...
import { depthInc, false0, depthZero, isAddrNo, addrNo, weak, strong, formStrong, assumeIsType, } from "../graph/graph-heap2.js";
...
const arFail = false;
const arMark = true;
export function isArUpdate(ar) {
    return typeof ar === "number";
}
export function mkPrims() {
    const builtinsG = new Map;
    const builtinsByAddr = new Map;
    const builtinTmOps = new Map;
    const builtinTyOps = new Map;
    const decls = [];
    let primEnv = null;
    function get(nameG) {
    ...
    function init(h, subst, inst, predicates, tis, ga) {
        const env = mkEnv();
        const { tyOp1, tyOp2, isTyOp, arg0_ty, arg1_ty, tyPrim0, tyPrim1, tyPrim2, tyPrim3, isTyPrim, isPrim, tmOp1, tmOp2, isTmOp, arg0_tm, arg1_tm, tyPair, isTyPair, hd_ty, tl_ty, tyFun, isTyFun, no_ty, yes_ty, dom_ty, cod_ty, isTySingleStr, value_ty, tmLam, isTmLam, pat_tm, body_tm, } = h;
        const typeType = h.tyPrim0("Type", depthZero);
        let defPrimitiveType = (name) => {
            const type = h.tyPrim0(name, depthZero);
            env.set(name, type);
            return type;
        };
        let defType = (name, defn) => {
        ...
        function builtinId(name, paramStrengths, type, action) {
            ...
            const builtin = { name, nameG, arity, paramStrengths, type, action };
            ...
            env.set(name, tm);
            const pat = { tag: "EVar", loc: nilLoc, name, tm, synTy, ctxTy: synTy, rc: patRc, torp: "Pat" };
            // TODO ? have "instBuiltin" build and return an ExprTypeGraph expression too ?
            const defn = { tag: "EVar", loc: nilLoc, name, tm, synTy, ctxTy, rc: defnRc, torp: "Term" };
            decls.push([pat, defn]);
        }
        function builtinTermOp(name, paramStrengths, type, action) {
            const arity = paramStrengths.length;
            const nameG = `(${name})`;
            const builtin = { name, nameG, arity, paramStrengths, type, action };
            assert.isTrue(!builtinsG.has(nameG));
        ...
        function builtinTypeOp1(name, paramStrengths, type, action) {
            const arity = paramStrengths.length;
            const nameG = `{${name}}`;
            const builtin = { name, nameG, arity, paramStrengths, type, action };
            assert.isTrue(!builtinsG.has(nameG));
        ...
        function builtinTypeOp2(name, paramStrengths, type, action) {
            const arity = paramStrengths.length;
            const nameG = `{${name}}`;
            const builtin = { name, nameG, arity, paramStrengths, type, action };
            assert.isTrue(!builtinsG.has(nameG));
        ...
        function builtinTODO(name, paramStrengths, type) {
        ...
        function unrollRecursiveType(aAddr) {
            const a = dOf(aAddr);
        ...
        builtinTypeOp2("|", [weak, weak], parseTy("Type -> Type -> Type"), (depth, [a0, b0]) => {
            ...
            if (h.isPrim("Any", a) || h.isPrim("Any", b))
                return h.tyPrim0("Any", depthZero);
            if (h.isTyVoid(a))
                return b;
            if (h.isTyVoid(b))
                return a;
            if (h.isPrim("Nil", a) && h.isPrim("Nil", b))
                return a;
            if (h.isPrim("Bool", a) && h.isPrim("Bool", b))
                return a;
            if (h.isPrim("Int", a) && h.isPrim("Int", b))
                return a;
            if (h.isPrim("Str", a) && h.isPrim("Str", b))
                return a;
            if (h.isPrim("Str", a) && h.isPrim("Char", b))
                return a;
            if (h.isPrim("Char", a) && h.isPrim("Str", b))
                return b;
            if (h.isPrim("Type", a) && h.isPrim("Type", b))
                return a;
        ...
        builtinTypeOp2("&", [weak, weak], parseTy("Type -> Type -> Type"), (depth, [aAddr, bAddr]) => {
            ...
            if (isPrim("Self", a) && isPrim("Self", b)) {
                ...
                if (h.isTmLam(a0) && h.isTmLam(b0)) {
                    ...
                    const resultDepth = Math.max(aDepth, bDepth);
            ...
            if (isPrim("Nil", a) && isPrim("Nil", b))
                return a;
            // if (a.tag === "TBool" && b.tag === "TBool") return boolT
            if (isPrim("Bool", a) && isPrim("Bool", b))
                return a;
            // if (a.tag === "TInt" && b.tag === "TInt") return intT
            if (isPrim("Int", a) && isPrim("Int", b))
                return a;
            // if (a.tag === "TStr" && b.tag === "TStr") return strT
            if (isPrim("Str", a) && isPrim("Str", b))
                return a;
            ...
            if (isPrim("Type", a) && isPrim("Type", b))
                return a;
            ...
            // }
            let aPossiblyUnrolled = a;
            let bPossiblyUnrolled = b;
            if (isPrim("Fix", a)) {
            ...
            }
            {
                ...
                const isDisjoint2 = (a) => isTySingleStr(a) || isTyFun(a) || isDisjoint1(a);
        ...
        builtinTypeOp2("\\", [weak, weak], parseTy("Type -> Type -> Type"), (depth, [aAddr, bAddr]) => {
            ...
            if (isPrim("{<:}", a)) {
                const aSub = dOf(arg0_ty(a));
                const aSup = dOf(arg1_ty(a));
                if (equalTypes(aSub, b))
                    return tyPrim0("Void", depthZero);
                if (equalTypes(aSup, b))
                    return tyPrim0("Void", depthZero);
            ...
            //     return voidT
            // }
            if (isTySingleStr(a) && isPrim("Type", b))
                return tyPrim0("Void", depthZero);
            ...
            //     return a
            // }
            if ((isPrim("List", a) || isTyPair(a)) && isDisjoint2(b))
                return a;
        ...
        builtinId("Tl", [weak], parseTy("Type -> Type"), (depth, [a0] /*, ki*/) => {
            // ki = kiTrue
            const a = h.directAddrOf(a0);
        ...
        builtinId("Dom", [weak], parseTy("Type -> Type"), (depth, args) => {
        ...
        builtinId("termOf", [weak], parseTy("{ A : Type } -> A"), (depth, [a0]) => {
            const a = h.directAddrOf(a0);
            if (isPrim("Single", a)) {
        ...
        builtinId("Apply", [weak, weak], parseTy("Type -> Type -> Type"), (depth, [funcTyAddr, argTyAddr]) => {
        ...
        builtinId("InverseApply", [weak, weak], parseTy("Type -> Type -> Type"), (depth, [funcAddr, argAddr]) => {
            ...
            if (h.isPrim("Self", func) && h.isTyFun(arg)) {
                ...
                if (h.isTmLam(funcSelfLam)) {
                    ...
                    const substEnv = new Map;
                    const selfVar = h.tmVar([], var1Depth, fun1);
                    substEnv.set(h.pathKey_root, selfVar);
                    const fun2 = subst.substTmTy(appDepth, selfLamVarDepth, substEnv, fun1, funcSelfBody);
        ...
        function unknownTerm(ty, depth) {
            depth ??= h.depthOf(ty);
            return h.tmOp0("unknown", depth, ty);
        }
        function mkBoolBoolToBool(name, op) {
            builtinTermOp(name, [weak, weak], parseTy("Bool -> Bool -> Bool"), (depth, [a, b]) => {
                const aTm = dOf(a);
                const bTm = dOf(b);
        ...
        function derefTuple2(addr, result) {
            const a = dOf(addr);
            if (!h.isTmPair(a))
                return false;
        ...
        function mkOpDatum2(name, aJsTypeOf, bJsTypeOf, aType, bType, resultType, op) {
        ...
        function mkDatum1Action(aJsTypeOf, op) {
            return (depth, [a0]) => {
                const a = dOf(a0);
                if (h.isTmDatum(a)) {
                    const aVal = h.datum_tm(a);
        ...
        function mkDatum2Action(aJsTypeOf, bJsTypeOf, op) {
            return (depth, [a0, b0]) => {
                const a = dOf(a0);
                const b = dOf(b0);
                if (h.isTmDatum(a) && h.isTmDatum(b)) {
        ...
        // { { a : Int } -> { b : Int } -> _ <: Int }
        // { A @ Int -> B @ Int -> A + B <: Int }
        builtinTermOp("+", [weak, weak], parseTy("Int -> Int -> Int"), mkDatum2Action("number", "number", (a, b) => a + b));
        builtinTermOp("-", [weak, weak], parseTy("Int -> Int -> Int"), mkDatum2Action("number", "number", (a, b) => a - b));
        builtinTermOp("*", [weak, weak], parseTy("Int -> Int -> Int"), mkDatum2Action("number", "number", (a, b) => a * b));
        builtinTermOp("<", [weak, weak], parseTy("Int -> Int -> Bool"), mkDatum2Action("number", "number", (a, b) => a < b));
        builtinTermOp(">", [weak, weak], parseTy("Int -> Int -> Bool"), mkDatum2Action("number", "number", (a, b) => a > b));
        builtinTermOp("<=", [weak, weak], parseTy("Int -> Int -> Bool"), mkDatum2Action("number", "number", (a, b) => a <= b));
        builtinTermOp(">=", [weak, weak], parseTy("Int -> Int -> Bool"), mkDatum2Action("number", "number", (a, b) => a >= b));
        ...
        builtinTermOp("|=", [weak, weak], parseTy("Bool -> A @ Any -> [] | [A]"), (depth, [a0, b0]) => {
            ...
            if (h.isTmDatum(a)) {
                const aVal = h.datum_tm(a);
                if (aVal === true)
                    return h.tmPair(b, nilTerm, depth);
                if (aVal === false)
                    return nilTerm;
            }
            return null;
        ...
        builtinId("if", [weak, weak], primType_if, (depth, [a0, b0], targetForm) => {
            const a = dOf(a0);
            const b = dOf(b0);
            if (h.isTmDatum(a)) {
                const aVal = h.datum_tm(a);
        ...
        builtinId("if2", [weak, weak], primType_if, (depth, [a0, b0], targetForm) => {
            const a = dOf(a0);
            const b = dOf(b0);
            if (h.isTmDatum(a)) {
                const aVal = h.datum_tm(a);
        ...
        builtinId("ifNil", [weak, weak], parseTy("A @ Any -> K @ [-> Any, { A \\ Nil } -> Any] -> (Hd K) [] | (Hd (Tl K)) { A \\ [] }"), mkIfPrim((a) => h.isTmDatum(a) && h.datum_tm(a) === null));
        builtinId("ifPair", [weak, weak], parseTy("A @ Any -> K @ [[Any ,, Any] -> Any, { A \\ [Any ,, Any] } -> Any] -> (Hd K) [(Hd A), (Hd (Tl A))] | (Hd (Tl K)) { A \\ [Any ,, Any] }"), mkIfPrim((a) => h.isTmPair(a)));
        builtinId("ifStr", [weak, weak], parseTy("A @ Any -> K @ [Str -> Any, { A \\ Str } -> Any] -> (Hd K) Str | (Hd (Tl K)) { A \\ Str }"), mkIfPrim((a) => h.isTmDatum(a) && typeof (h.datum_tm(a)) === "string"));
        builtinId("ifInt", [weak, weak], parseTy("A @ Any -> K @ [Int -> Any, { A \\ Int } -> Any] -> (Hd K) Int | (Hd (Tl K)) { A \\ Int }"), mkIfPrim((a) => h.isTmDatum(a) && typeof (h.datum_tm(a)) === "number"));
        builtinId("ifBool", [weak, weak], parseTy("A @ Any -> K @ [Bool -> Any, { A \\ Bool } -> Any] -> (Hd K) Bool | (Hd (Tl K)) { A \\ Bool }"), mkIfPrim((a) => h.isTmDatum(a) && typeof (h.datum_tm(a)) === "boolean"));
        ...
        function tmTuple2(elem0, elem1, depth) {
        ...
        builtinId("strOrd", [weak], parseTy("Str -> Int"), mkDatum1Action("string", (a) => a.charCodeAt(0)));
        // TODO ? Rename: charFromInt
        builtinId("strChr", [weak], parseTy("Int -> Char"), mkDatum1Action("number", (a) => String.fromCharCode(a)));
        ...
        for (const [name, defn] of declStrs) {
            ...
            try {
                ...
                const pat = { tag: "EVar", loc: nilLoc, name, tm, synTy, ctxTy: synTy, rc, torp: "Pat" };
                // TODO ? have "instBuiltin" build and return an ExprTypeGraph expression too ?
                const defn = defnExpr;
                decls.push([pat, defn]);
            }
...
//# sourceMappingURL=graph-primitives.js.map