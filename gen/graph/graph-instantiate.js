import { eList, isLambdaExpr, isPatTypeAnnotated } from "../syntax/expr.js";
import { isAddrYes, addrNo, depthInc, booleanToBool, depthZero, isAddrNo, isTyOp2Name, addrTypeType, assumeIsType, } from "../graph/graph-heap2.js";
import { assert } from "../utils/assert.js";
// import { assumeIsType } from "../graph/graph-ti.js";
import { isAlpha } from "../syntax/scan.js";
export function mkInstantiate(h, gr, pred, ti) {
    ...
    function tyTail(pairTy, depth) {
        if (isAddrNo(pairTy))
            return addrNo;
        return h.tyPrim1("Tl", pairTy, depth);
    }
    function typeOfDatum(value) {
        if (value === null) {
            return tyNil;
        }
        switch (typeof value) {
    ...
    function applyFunTy(depth, funTy, argTm, argTy) {
    ...
    function instTypeValue(prims, env, depth, type) {
        switch (type.tag) {
            case "TVoid":
                return tyVoid;
            case "TNil":
    ...
    function typeLetPat(prims, env, depth, path, pat, ctxTy, performTypeCheck) {
        return tlp(path, pat, ctxTy);
        function tlp(path, pat, ctxTy) {
            switch (pat.tag) {
                case "EVar": {
                    return ctxTy;
                }
                ...
                case "EType": {
                    ...
                    const tyTy = instTerm(prims, env, depth, pat.type, ctxTy, performTypeCheck);
    ...
    function instLetPat(prims, env, depth, pat, defn, ctxTy, isCtxAnnot, performTypeCheck) {
        const bindings = [];
        ilp(pat, defn, ctxTy, isCtxAnnot);
        return bindings;
        function ilp(pat, defn, ctxTy, isCtxAnnot) {
        ...
        function ilp2(pat, defn, ctxTy, isCtxAnnot) {
            switch (pat.tag) {
                case "EVar": {
    ...
    function typeLambdaPat(prims, env, depth, path, pat, performTypeCheck) {
        const tp = (path, pat) => typeLambdaPat(prims, env, depth, path, pat, performTypeCheck);
        switch (pat.tag) {
            case "EVar": {
                return tyAny;
            }
    ...
    function instLambdaPat(prims, env, depth, path, pat0, tyVar, ctxTy, isCtxAnnot, performTypeCheck) {
        const noTyVar = addrNo;
        const patAddr = ilp(path, pat0, tyVar, ctxTy, isCtxAnnot);
        return patAddr;
        function ilp(path, pat0, tyVar, ctxTy, isCtxAnnot) {
        ...
        function ilp2(path, pat0, tyVar, ctxTy, isCtxAnnot) {
            const pat = pat0;
            switch (pat.tag) {
                case "EVar": {
    ...
    function instTerm(prims, env, depth, expr, ctxTy, performTypeCheck) {
        const term = instTerm1(env, depth, expr, ctxTy);
        return term;
        function instTerm1(env, depth, expr, ctxTy) {
        ...
        function instTerm2(env, depth, expr, ctxTy) {
            ...
            switch (expr.tag) {
                ...
                case "EApply": {
                    ...
                    switch (expr.op) {
                        ...
                        case "<|": {
                            ...
                            const argCtxTy = isLambdaExpr(expr.arg) && !isPatTypeAnnotated(expr.arg.pat)
                ...
                case "ELambdaNo": {
                    ...
                    if (no) {
                        ...
                        if (expr_pat1.tag === "ETermBrackets") {
                            addDummyTypes(expr_pat1);
                            expr_pat1 = expr_pat1.expr;
                        }
                        const [expr_pat, tyVarName, tyVarType] = expr_pat1.tag === "EType" && expr_pat1.type.tag === "EAs"
                            ? [expr_pat1.expr, expr_pat1.type.name, expr_pat1.type.expr]
                            : [expr_pat1, null, null];
                        if (expr_pat1.tag === "EType" && expr_pat1.type.tag === "EAs") {
                            addDummyTypes(expr_pat1);
                            addDummyTypes(expr_pat1.type);
                            addDummyTypes(expr_pat1.expr);
                        }
                        const env2 = env.clone();
                        const patCtxTy = h.tyPrim1("Dom", ctxTy, depth);
                        const patGuardTy = typeLambdaPat(prims, env, varDepth, [], expr.pat, performTypeCheck);
                        ...
                        const isCtxAnnot = false;
                        const patAddr = instLambdaPat(prims, env2, varDepth, [], expr_pat, tyVarName === null ? addrNo : tyVar, patCtxTy2, isCtxAnnot, performTypeCheck);
                ...
                case "EType": {
                    const ty = instTerm1(env, depth, expr.type, tyType);
    ...
    function instType(prims, env, depth, expr0, ctxTy, performTypeCheck) {
        ...
        return ty;
        function instType1(env, depth, expr, ctxTy) {
        ...
        function instType2(env, depth, expr0, ctxTy) {
            const expr = expr0;
            switch (expr.tag) {
                case "EVar": {
    ...
    function instDecl(prims, env, depth, pat, defn, performTypeCheck) {
...
//# sourceMappingURL=graph-instantiate.js.map