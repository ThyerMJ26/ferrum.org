...
function page() {
    return pageBuild((b) => {
        ...
        p("Ferrum is the solution, when the problem is too many forms of abstraction.", "(Or at least, that is what it aspires to be.)");
        ...
        ]), "New forms of abstraction can be added to a language for a number of reasons, two common ones are:", l(["Moving computation from run-time to compile-time,",
        ...
        p("The downside of multiple forms of abstraction, is that you may then need to write the essentially same code multiple times.", "Abstractions are created to avoid repetition.", "But, the act of introducing a new form of abstraction, changes the very thing we are trying to abstract over.", "Each time we add a new form of abstraction to avoid one form of repetition we risk (or inevitably cause?) a new potential form of repetition.", "Ferrum aims to provide a single form of abstraction in such a way that there is no need to introduce further forms of abstraction.");
        ...
        p("The benefits this brings are:", l([
        ...
        p("To provide better syntactic support for specific language constructs,", "(perhaps because writing everything in CPS (continuation-passing style) looks a bit odd),", "we can write an interpreter for an extended language, and specialize away the interpretive overhead.");
        p("Is this all too good to be true?", "Well, maybe.", "The fundamental tension between type-system expressability and decidability means we cannot have everything desirable simultaneously.");
        p("A language design cannot choose both expressability and decidability, but perhaps it can not choose at all.", "That is, the decision can be deferred to the user.", "We can have both, but not at the same time.", "When we want decidability, we must limit our expressiveness.", "When we want unbounded expressiveness, we must accept the type-checker might return \"unknown\".");
        p("The ideal type-system should be able to decide everything we wish to express.", "Most (practically all) typed programming languages start with a decidable type-system with limited expressiveness.", "Often, these will then go on to gain greater expressiveness.", "For example adding polymorphic-types to Java/Go, or adding dependent-types to Haskell.", "Each time the expressiveness of a decidable language is increased, the syntax is typically modified.", "The things which cannot be decided simply cannot be expressed.", "This is both good and bad.", "It's good because; so long as we follow the syntax of the language, we'll either get a working program, or a definitive type-error.", "It's bad because; the things which cannot be decided, cannot even be written down.", "Without a valid notation, it becomes difficult to even talk about that which cannot currently be typed.");
...
//# sourceMappingURL=what-is-ferrum.js.map