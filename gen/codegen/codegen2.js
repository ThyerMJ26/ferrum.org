...
import { eList } from "../syntax/expr.js";
// Tree
import { isPrimName } from "../tree/eval.js";
// CodeGen
import { jsCall, jsDatum, jsLambdaExpr, jsList, jsOp, jsVar, jsReturn, jsLambdaStmt, jsLet, freshVarName, jsExpr_print2 } from "./codegen-js.js";
import { pretty_printJs } from "./pretty-js.js";
function cg8_pat(pat, jsEnv, jsNameSrc) {
    let asPats = [];
...
function cg8_mat(pat, jsEnv, jsNameSrc) {
    switch (pat.tag) {
        case "EDatum": {
...
function cg8_asPats2(asPats, exprJs) {
    let result = asPats.reduce((result, [name, pat]) => {
        return jsCall(jsVar("rt.as"), [jsVar(name), jsLambdaExpr([pat], result)]);
    }, exprJs);
    return result;
}
function cg8_expr(exp, jsEnv, jsNameSrc) {
    let cg = (e) => cg8_expr(e, jsEnv, jsNameSrc);
    switch (exp.tag) {
        case "EVar": {
...
function cg8_decls(decls, jsEnv, jsNameSrc) {
    let jsStmts = [];
