...
import { anyT, pairT, ttiIsFalse, tiStructuralRelComp, typeDom, typeHd, typeTl, unknownT } from "../tree/types.js";
// CodeGen
import { jsCall, jsDatum, jsLambdaExpr, jsList, jsOp, jsVar, freshVarJs, jsReturn, jsLambdaStmt, jsLet, jsExpr_print, jsStmts_print } from "./codegen-js.js";
function cgty_pat(pat, jsEnv, jsNameSrc) {
    let asPats = [];
    function cg8_pat(pat) {
        switch (pat.tag) {
            case "EDatum": {
                return undefined;
            }
...
function typeIsPair(ty) {
    return ttiIsFalse(tiStructuralRelComp(ty, pairT(anyT, anyT), true));
}
function cgty_mat(ty, pat, jsEnv, jsNameSrc) {
    let tyU = unknownT;
    switch (pat.tag) {
        case "EDatum": {
...
function cgty_decls(decls, jsEnv, jsNameSrc) {
    let jsStmts = [];
