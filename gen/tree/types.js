...
import { assumeExprIsTyped, showExpConcise, patBindVars, exprFreeVars, visitAll } from "../syntax/expr.js";
import { evalExpr, evalNode, node, blockedValue, typeValue, termVarValue, lookupPrimEnv } from "../tree/eval.js";
import { showLoc, nilLoc } from "../syntax/token.js";
import { mk_MemoData } from "../tree/memoize.js";
...
function reportProgress(level, text, loc) {
    flushProgressReport();
...
let typeMemoData = mk_MemoData();
export function getTypeMemoData() {
    return typeMemoData;
}
function hcTy(ty) {
    // return ty
...
function addKnownInhabited(ty, ki) {
    let ty2 = { ...ty };
...
export function knownInhabited(ty) {
    switch (ty.tag) {
        case "TAs":
...
function notKnownInhabited(ty) {
    return addKnownInhabited(ty, false);
}
function typeParts2(type) {
...
export function showTypeDiff(out, typ1, typ2) {
    ...
    function showItem(item) {
        ...
        result += `${field} = ${tag} ${attrs.map(a => (JSON.stringify(ty[a]))).join(" ")}`;
        return result;
    }
    while (todo1.length !== 0 && todo2.length !== 0) {
        ...
        let vals1 = attrs1.map(a => t1[a]);
        let vals2 = attrs2.map(a => t2[a]);
        if (inSync1 && !inSync2) {
...
function findOpElems_All(ty, opName) {
    let result = [];
    let todo = [ty];
    while (todo.length !== 0) {
        let elem = todo.pop();
...
function findOpElems_Left(ty, opName) {
    let result = [];
...
function findOpElems_Right(ty, opName) {
    let result = [];
    while (ty.tag === "TRule" && ty.name === opName) {
...
export function showType3(type, maxDepth = null, next, showInhabitedFlag = true) {
    if (next !== undefined) {
        type = next(type);
    }
    let currentDepth = 0;
...
function showType2full(type) {
    return showType3(type, null);
}
export function showType2(type, next, showInhabitedFlag = true) {
    return showType3(type, 10, next, showInhabitedFlag);
}
function convertTypeToDisplayTree(ty, maxDepth) {
    let leaf = (value) => ({ tag: "Leaf", value: value });
    ...
    let branch = (children) => ({ tag: "Branch", children: children });
    let zipBrackets = (open, separator, close, children) => {
    ...
    function convert(ty, depth) {
        let conv = (ty) => convert(ty, depth + 1);
...
function showDisplayTree(dt, maxDepth, maxWidth) {
    ...
    switch (dt.tag) {
        ...
        case "Branch": {
            let parts = dt.children.map(c => [c[0], show(c[1], maxDepth1, maxWidth)]);
            let allFlat = parts.every(p => p[1].length === 1);
            if (allFlat) {
                let parts2 = [].concat(...parts.map(p => [p[0], ...p[1]]));
...
export function showType4(ty, maxDepth, maxWidth) {
    let dt = convertTypeToDisplayTree(ty, maxDepth);
    let lines = showDisplayTree(dt, maxDepth, maxWidth);
    let result = lines.join("\n");
    return result;
}
export function showType5(ty, indent, maxDepth, maxWidth) {
    let dt = convertTypeToDisplayTree(ty, maxDepth);
    let lines = showDisplayTree(dt, maxDepth, maxWidth);
...
export const typeT = knownInhabited({ tag: "TType" });
export const boolT = knownInhabited({ tag: "TBool" });
export const intT = knownInhabited({ tag: "TInt" });
export const charT = knownInhabited({ tag: "TChar" });
export const strT = knownInhabited({ tag: "TStr" });
export const anyT = knownInhabited({ tag: "TAny" });
export const nilT = knownInhabited({ tag: "TNil" });
export const voidT = notKnownInhabited({ tag: "TVoid" });
export const unknownT = notKnownInhabited({ tag: "TUnknown" });
export const errorT = notKnownInhabited({ tag: "TError" });
export const ioWorldT = ruleT("ioWorld", []);
export let relcompT = (a, b) => ruleT("relcompT", [a, b]);
export let unionT = (a, b) => ruleT("unionT", [a, b]);
export let maybeT = (a) => ruleT("unionT", [nilT, pairT(a, nilT)]);
export let intersectT = (a, b) => ruleT("intersectT", [a, b]);
export let applyT = (a, b) => ruleT("applyT", [a, b]);
export let rangeT = (a) => ruleT("rangeT", [a]);
export let domainT = (a) => ruleT("domainT", [a]);
export let hdT = (a) => ruleT("hdT", [a]);
export let tlT = (a) => ruleT("tlT", [a]);
// monomorphic functions
export function funT(argT, resultT) {
    return notKnownInhabited({ tag: "TFun", argType: argT, resultType: resultT });
}
// polymorphic functions
export function funPT(argTypeName, argT, resultT) {
...
export function funDT(tyVar, argT, resultT) {
...
// }
export function varT(v, ki = false) {
    if (!TRACK_TYPE_INHABITATION) {
        ki = false;
    }
    return hcTy({ tag: "TVar", name: v, ki: ki });
}
export function asVarT(v) {
    return varT(v, true);
}
export function pairT(h, t) {
...
export function superT(type, sub) {
...
export function selfT(name, body) {
    let fv = typeFreeVars(body);
    if (body.tag === "TSelf") {
...
export function typeFreeVars(ty) {
    let result = [];
    typeFreeVars1(ty, [], result);
    return result;
}
function typeFreeVars1(ty, boundVars, freeVars) {
    switch (ty.tag) {
        case "TAny":
        case "TInt":
        case "TChar":
...
export function trimEnviron(expr_decl, env) {
...
//#endregion
//#region Reduction
export function nodeToType(node) {
...
function unrollRecursiveType(ty) {
    if (ty.body.tag === "TRec") {
...
export function reduceTypeRule1(ruleName, args) {
...
export function substType(ty, varName, value, simplify) {
    let env = {};
...
export function evalTypeAnnot(expr, env) {
    switch (expr.tag) {
        case "EAs": {
            let env2 = { ...env };
...
export function evalTypeBrackets(expr, env) {
    switch (expr.tag) {
        case "EDatum": {
...
export function matchTypes(pat, ty) {
    switch (pat.tag) {
        case "TAs": {
...
//     // return ty
// }
function closePatType(name, ty) {
    function cpt(path, pathTy, ty) {
        // if (equalType(path, ty)) {
        //     return anyT
        // }
        if (equalTypes2(path, ty, false)) {
...
export function convertPatTypeClosed(pat) {
    switch (pat.tag) {
        case "TAs":
...
function convertPatTypeOpen(ty) {
    switch (ty.tag) {
        case "TAs": {
...
function substTypeEnv(ty, env, simplify) {
    return substTypeEnv1(ty, env, {}, simplify);
}
function substTypeEnv1(ty, env, freeVars, simplify) {
    if (Object.keys(env).length === 0) {
        return ty;
    }
    simplify = true;
    // simplify = false
    let ste = (ty, env, freeVars) => {
...
export function applyTypes1(func, arg, checkDomain = true) {
    checkDomain = false;
    switch (func.tag) {
        case "TAs":
        ...
        case "TRule": {
            ...
            switch (func.name) {
                case "intersectT": {
                    ...
                    // let st = showType2
                    let st = (ty) => showType4(ty, null, 160);
...
export function typeErrorDetected(expr) {
...
export function typeExprBidir(expr, contextTy, env, topLevel) {
    const teb = typeExprBidir;
    function returnResult(ty1) {
    ...
    try {
        ...
        switch (expr.tag) {
            ...
            case "ELambdaMaybe": {
                ...
                let bodyExp = teb(expr.body, bodyTy, newEnv, false);
                let lamTyp1 = funT(getTypeOrError(pat2), pairT(getTypeOrError(bodyExp), nilT));
                let lamTyp2 = funT(mf.typeRelComp(domTy, maybePatTy), nilT);
                let lamTyp = mf.intersectTypes(lamTyp1, lamTyp2);
                let superCodomainTyp = mf.unionTypes(nilT, pairT(anyT, nilT));
                let lamSubTyp = subT(lamTyp, funT(domTy, superCodomainTyp));
            ...
            case "ELambdaNo": {
                ...
                let bodyExp = teb(expr.body, bodyTy, newEnv, false);
                let lamTyp1 = funT(getTypeOrError(pat2), getTypeOrError(bodyExp));
                let lamTyp2 = funT(mf.typeRelComp(domTy, maybePatTy), nilT);
                let lamTyp = mf.intersectTypes(lamTyp1, lamTyp2);
                let superCodomainTyp = mf.unionTypes(nilT, anyT);
                let lamSubTyp = subT(lamTyp, funT(domTy, superCodomainTyp));
...
function typePatBindBidir(pat, val, valTy, annotTy, env, topLevel) {
    let tpb = (pat, val, valTy, annotTy) => typePatBindBidir(pat, val, valTy, annotTy, env, topLevel);
    if (valTy !== null && annotTy !== null) {
        throw new Error("oops");
    }
    let conTy = anyT;
    ...
    function assignType(ty1, ty2 = null) {
...
function typeTypePatBindBidir(expr, env, topLevel) {
    const ttpbb = (expr) => typeTypePatBindBidir(expr, env, topLevel);
    function assignType(ty1, ty2, tc) {
        ty2 ??= typeT;
        if (tc === undefined) {
    ...
    switch (expr.tag) {
        ...
        case "EVar": {
            ...
            let [ty, tyT] = env[expr.name];
            let tc = tyT.tag === "TType" || tyT.tag === "TSingleType" ? "ok" : "error";
            const ty1 = tyT;
            const ty2 = typeT;
        ...
        case "ETermBrackets": {
            ...
            let tc = expr2.ty1.tag === "TType" || expr2.ty1.tag === "TSingleType" ? "ok" : "error";
            const ty1 = expr2.ty1;
            const ty2 = typeT;
            assignType(ty1, ty2, tc);
            return envEmpty;
...
function typeTypeBidir(expr, env, topLevel) {
    const ttb = typeTypeBidir;
    function returnResult2(ty1, tc) {
        const ty2 = typeT;
        tc ??= typeCheck(ty1, ty2, expr.loc);
        const expr2 = expr;
        expr2.ty1 = ty1;
    ...
    switch (expr.tag) {
        ...
        case "EList": {
            ...
            let ty = tail === null ? singleTypeT(nilT) : tail?.ty1;
...
export function typeDeclBidir(expr_decl, env, topLevel) {
    let [pat, defn] = expr_decl;
    ...
    let declVal = node(blockedValue("let"));
    let declTy = getTypeOrError(declExp);
    if (topLevel) {
...
function typeDeclsBidir(expr_decls, env, topLevel) {
    let newEnv = { ...env };
    let decls = [];
    for (let expr_decl of expr_decls) {
...
export function maxTypeCheckResult(a) {
    let resultLevel = 0;
    for (let t of a) {
...
// }
// checks if we can use an "a" as a "b"
function typeCheck2_noMemo(a, b) {
...
export function typeCheck(a, b, loc) {
    // b = knownInhabited(b)
...
export function typeCheckTorP(torp, ty1, ty2, loc) {
    switch (torp) {
        case "Term":
...
function typeCheckPat(ty) {
    // check for VERY obvious uninhabitation
    let tc = ty.tag === "TVoid" ? "error" : "ok";
    return tc;
...
const ttiUnknown = [false, false];
const ttiDontKnow = [false, false]; // TODO ? distinguish between don't know and can't know ?
const ttiCantKnow = [false, false]; // TODO ?
const ttiFalse = [true, false];
const ttiTrue = [false, true];
const ttiContradiction = [true, true];
function ttiKnown(ti) {
    return ti ? ttiTrue : ttiFalse;
}
function TypeInhabitationContradictionDetected(msg = "") {
    throw new Error("Type Inhabitation Contradiction Detected");
    // console.log(`Type Inhabitation Contradiction Detected ${msg}`)
}
function ttiAnd([aF, aT], [bF, bT]) {
    return [aF || bF, aT && bT];
}
function ttiAndImp(a, b) {
...
function ttiOr([aF, aT], [bF, bT]) {
    return [aF && bF, aT || bT];
}
function ttiOrImp(a, b) {
    if (ttiIsTrue(a) && ttiIsFalse(b)) {
        TypeInhabitationContradictionDetected();
        return ttiContradiction;
    }
...
function ttiNot([aF, aT]) {
    // return [!aF, !aT]
    return [aT, aF];
}
export function ttiIsFalse([tF, tT]) {
    if (tF && tT) {
        TypeInhabitationContradictionDetected();
    }
    return tF;
}
function ttiIsTrue([tF, tT]) {
    if (tF && tT) {
        TypeInhabitationContradictionDetected();
    }
    return tT;
...
let ttieConst = (ti) => ({ tag: "Const", result: ti });
let ttieRef = (tim, tyA) => ({ tag: "Ref", a: timKey(tim, tyA) });
let ttieAnd = (tyA, tyB) => ({ tag: "And", a: tyA, b: tyB });
let ttieAndImp = (tyA, tyB) => ({ tag: "AndImp", a: tyA, b: tyB });
let ttieOr = (tyA, tyB) => ({ tag: "Or", a: tyA, b: tyB });
let ttieOrImp = (tyA, tyB) => ({ tag: "OrImp", a: tyA, b: tyB });
let ttieNot = (tyA) => ({ tag: "Not", a: tyA });
let ttieFalse = ttieConst(ttiFalse);
let ttieTrue = ttieConst(ttiTrue);
let ttieUnknown = ttieConst(ttiUnknown);
let ttieAndRefs = (tim, tyA, tyB) => ({ tag: "And", a: ttieRef(tim, tyA), b: ttieRef(tim, tyB) });
let ttieAndImpRefs = (tim, tyA, tyB) => ({ tag: "AndImp", a: ttieRef(tim, tyA), b: ttieRef(tim, tyB) });
let ttieOrRefs = (tim, tyA, tyB) => ({ tag: "Or", a: ttieRef(tim, tyA), b: ttieRef(tim, tyB) });
let ttieOrImpRefs = (tim, tyA, tyB) => ({ tag: "OrImp", a: ttieRef(tim, tyA), b: ttieRef(tim, tyB) });
let ttieAndUnknown = (tim, tyA, tyB) => ({ tag: "And", a: { tag: "And", a: ttieRef(tim, tyA), b: ttieRef(tim, tyB) }, b: ttieConst(ttiUnknown) });
let ttieOrUnknown = (tim, tyA, tyB) => ({ tag: "Or", a: { tag: "Or", a: ttieRef(tim, tyA), b: ttieRef(tim, tyB) }, b: ttieConst(ttiUnknown) });
export function timGet(tim, key) {
    // return tim[key]
    return tim.get(key);
}
function timSet(tim, key, entry) {
    // return tim[key] = entry
    return tim.set(key, entry);
}
function timSize(tim) {
    // return Object.keys(tim).length
    return tim.size;
}
function timHas(tim, key) {
...
export let globalTIMemo = newTiMap();
...
function tiStructural(ty, assumeVarsInhabited = false) {
...
function tiStructural1(ty, assumeVarsInhabited = false) {
    let tis = (ty) => tiStructural(ty, assumeVarsInhabited);
    if (TRACK_TYPE_INHABITATION) {
        if (ty.ki) {
            return ttiTrue;
        }
...
let structuralRule = (tim, ty) => {
    let ti = tiStructural1(ty);
    return ttieConst(ti);
};
let structuralRefRule = (tim, ty) => {
    switch (ty.tag) {
        case "TAny":
        case "TBool":
        case "TInt":
...
export function tiStructuralRelComp(a, b, assumeVarsInhabited = false) {
    let tis = (ty) => tiStructural(ty, assumeVarsInhabited);
    let tisRc = (a, b) => tiStructuralRelComp(a, b, assumeVarsInhabited);
    let tisIn = (a, b) => tiStructuralIntersect(a, b, assumeVarsInhabited);
    if (equalType(a, b)) {
        return ttiFalse;
    }
...
function tiStructuralIntersect(a, b, assumeVarsInhabited = false) {
    let tis = (ty) => tiStructural(ty, assumeVarsInhabited);
    let tisRc = (a, b) => tiStructuralRelComp(a, b, assumeVarsInhabited);
    let tisIn = (a, b) => tiStructuralIntersect(a, b, assumeVarsInhabited);
    if (equalType(a, b)) {
...
// { A.single \ { B0 | B1 } }
let rc_SingleA_UnionB_Rule = (tim, ty) => {
    if (ty.tag === "TRule" && ty.name === "relcompT") {
        let [a, b] = ty.args;
...
// }
// { A \ { B | { C \ D }  } } -> { { A | D } \ { B \ C } }
let rcUnionRelCompBRule = (tim, ty) => {
    if (ty.tag === "TRule" && ty.name === "relcompT") {
        let [a, b] = ty.args;
...
// { (Codomain { A0 & A1 }) \ B }
let rcCodomainIntersectARule = (tim, ty) => {
    if (ty.tag === "TRule" && ty.name === "relcompT") {
        let [a, b] = ty.args;
...
let rcFuncRule = (tim, ty) => {
    if (ty.tag === "TRule" && ty.name === "relcompT") {
        let [a, b] = ty.args;
        if (a.tag === "TFun" && b.tag === "TFun") {
            let tDom = notKnownInhabited({ tag: "TRule", name: "relcompT", args: [b.argType, a.argType] });
            ...
            let tRng = notKnownInhabited({ tag: "TRule", name: "relcompT", args: [app, b.resultType] });
...
// { A \ { B.dom -> B.rng } }
let rcFuncBRule = (tim, ty) => {
    if (ty.tag === "TRule" && ty.name === "relcompT") {
        let [a, b] = ty.args;
        if (b.tag === "TFun") {
            // if (a.tag === "TFun" && b.tag === "TFun") {
            let aFun = typeRelComp0(a, funT(voidT, anyT));
            let tDom = notKnownInhabited({ tag: "TRule", name: "relcompT", args: [b.argType, typeDom(a)] });
...
let rcFuncIntersectBRule = (tim, ty) => {
    if (ty.tag === "TRule" && ty.name === "relcompT") {
        let [a, b] = ty.args;
        if (b.tag === "TRule" && b.name === "intersectT") {
...
// { { A.dom -> A.rng } & { B.dom -> B.rng } }
let inFuncRule = (tim, ty) => {
    if (ty.tag === "TRule" && ty.name === "intersectT") {
        let [a, b] = ty.args;
        if (a.tag === "TFun" && b.tag === "TFun") {
            let appA = applyTypes(a, convertPatTypeOpen(b.argType));
            let appB = applyTypes(b, convertPatTypeOpen(a.argType));
            let tRngA = notKnownInhabited({ tag: "TRule", name: "relcompT", args: [appA, b.resultType] });
            let tRngB = notKnownInhabited({ tag: "TRule", name: "relcompT", args: [appB, a.resultType] });
...
let rcApplyIntersectyRule = (tim, ty) => {
    if (ty.tag === "TRule" && ty.name === "relcompT") {
        let [a, b] = ty.args;
...
let rcApplyInverseDomain = (tim, ty) => {
    if (ty.tag === "TRule" && ty.name === "relcompT") {
        let [a, b] = ty.args;
...
function tiMergeValues(vals) {
    let result = ttiUnknown;
    let anyNulls = false;
    let causeKnown = [];
    let causeUnknown = [];
    vals.forEach((val, i) => {
...
function timKey(tim, ty) {
    ...
    if (!timHas(tim, key)) {
        ...
        let entry = { typeName: key, typeNum: typeNum, value: null, exprs: null, cause: [] };
        // tim[key] = entry
        timSet(tim, key, entry);
    }
    return key;
}
function tiExprChildren(tie) {
...
let tiConsoleLog = (mkMsg) => { };
export let tiStats = {
    totalIterCount: 0,
    totalExpansionCount: 0
};
function tiCalc(tim, ty) {
    ...
    try {
        ...
        let entryTy = typeMemoData.getData(entry.typeName);
        ...
        while (entry.value === null) {
            ...
            toReduce.forEach(key => {
                ...
                let expr_values = entry.exprs.map(([ruleName, exp]) => {
                    let result = [ruleName, exp, tiEvalExpr(tim, exp)];
                ...
                if (value !== null) {
                    ...
                    let entryType = typeMemoData.getData(entry.typeName);
...
export function showTIExpr(tim, tie) {
    let stie = (tie) => showTIExpr(tim, tie);
    switch (tie.tag) {
        case "Const":
...
export function tiShowCause(tim, ty, log) {
    let todo = [];
    let key1 = timKey(tim, ty);
    todo.push([key1, 0]);
    let visited = {};
...
export function tiSummaryGraph(tim, ty) {
    let summaryGraph = new Map();
    let todo = [];
    let key1 = timKey(tim, ty);
    todo.push(key1);
    let visited = new Map;
...
let hdDepth = 0;
export function typeHd(t, ki = false) {
    hdDepth += 1;
...
export function typeHd1(t, ki = false) {
...
export function typeTl(t, ki = false) {
    if (ASSUME_ALL_PAIRS_INHABITED) {
        ki = true;
    }
    let ruleName = "tlT";
...
export function typeTupleMap(f, t) {
    let ruleName = "tupleMapT";
...
export function typeDom(t) {
    let dom = typeDom1(t);
    return dom;
}
export function typeDom1(t) {
    let ruleName = "domainT";
    let arg = t;
    switch (arg.tag) {
        case "TFun": {
...
export function typeRng(t) {
    let rng = typeRng1(t);
    return rng;
}
export function typeRng1(t) {
    let ruleName = "rangeT";
    let arg = t;
    switch (arg.tag) {
        case "TFun": {
...
function equalTypes2(a, b, checkKi) {
    ...
    for (let i = 0; i !== aFields.length; i++) {
        ...
        let aFieldVal = a[fieldName];
        let bFieldVal = b[fieldName];
    ...
    for (let i = 0; i !== aElems.length; i++) {
        ...
        let aElem = a[elemName];
        let bElem = b[elemName];
...
function typeContainsTypeVar_noMemo(ty) {
    let [tag, fields, children] = typeParts2(ty);
    if (tag === "TVar" || tag === "TAs") {
        return true;
    }
    for (let child of children) {
        if (typeContainsTypeVar(ty[child])) {
            return true;
        }
    }
    return false;
...
export function typeRelComp0(a, b) {
...
export function typeRelComp1(a, b) {
    let rc = typeRelComp1;
    if (a.tag === "TVoid" || b.tag === "TAny") {
        return voidT;
    }
...
// }
export function typeUnion0(a, b) {
    return ruleT("unionT", [a, b]);
}
export function unionTypes1(a, b) {
    if (equalType(a, b)) {
        return a;
    }
    if (a.tag === "TError") {
    ...
    if (a.tag === "TNil" && b.tag === "TNil")
        return nilT;
    if (a.tag === "TBool" && b.tag === "TBool")
        return boolT;
    if (a.tag === "TInt" && b.tag === "TInt")
        return intT;
    if (a.tag === "TStr" && b.tag === "TStr")
        return strT;
    if (a.tag === "TStr" && b.tag === "TChar")
        return strT;
    if (a.tag === "TChar" && b.tag === "TStr")
        return strT;
    if (a.tag === "TType" && b.tag === "TType")
        return typeT;
    if (a.tag === "TPair" && b.tag === "TPair") {
    ...
    if (a.tag === "TList" && b.tag === "TList")
        return listT(unionTypes(a.elem, b.elem));
    if (a.tag === "TList" && b.tag === "TNil")
        return a;
    if (a.tag === "TNil" && b.tag === "TList")
        return b;
    if (a.tag === "TList" && b.tag === "TPair" && b.tl.tag === "TList")
        return listT(unionTypes(a.elem, unionTypes(b.hd, b.tl.elem)));
    if (a.tag === "TPair" && a.tl.tag === "TList" && b.tag === "TList")
        return listT(unionTypes(unionTypes(a.hd, a.tl.elem), b.elem));
    if (a.tag === "TFun" && b.tag === "TFun") {
...
export function unionTypeList(args) {
    let result = voidT;
    args.forEach(arg => {
        result = unionTypes(result, arg);
    });
...
function tryTypeIntersect(a, b) {
    ...
    if (a1.tag === "TList" && b1.tag === "TNil")
        return nilT;
    if (a1.tag === "TNil" && b1.tag === "TList")
        return nilT;
    if (a1.tag === "TPair" && b1.tag === "TNil")
        return voidT;
    if (a1.tag === "TNil" && b1.tag === "TPair")
        return voidT;
    // if (a.tag === "TFun" && b.tag === "TFun") {
...
export function intersectTypes(a, b) {
    // let [ok, ty] = reduceTypeRule("intersectT", [a, b])
    let [ok, ty] = reduceTypeRule("intersectT", [a, b]);
    return ty;
...
// }
export function typeIntersect0(a, b) {
    // return ruleT("intersectT", [a, b])
    return ruleT("intersectT", [a, b]);
}
function intersectTypeList(args) {
    let result = anyT;
    args.forEach(arg => {
...
//     return result
// }
export function countTypeErrors(exp) {
...
export function collectTypeErrors(exp) {
    let result = [];
    let call = (field, exp) => {
        if (exp instanceof Array) {
            return;
...
export function collectUnionTypes(ty, result) {
...
export function collectIntersectTypes(ty, result) {
...
export function disjoinTypes0(tys) {
    let result = [];
    tys.forEach(ty1 => {
...
export function disjoinTypes(tys) {
    let prevLength;
    do {
...
export function typeRemoveSuperSub(ty) {
...
export function showTypeWithoutSubSuperBounds1(ty) {
    let ty2 = typeRemoveSuperSub(ty);
    let result = showType2(ty2);
    return result;
}
export function showTypeWithoutSubSuperBounds(ty) {
    let next = (t) => {
        switch (t.tag) {
            case "TSub":
            case "TSuper": {
