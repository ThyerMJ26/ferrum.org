import { node, atomicValue, closureValue, typeValue, evalNode, pairValue, patMatch, evalExpr, showMbLoc, showValueFe, primValue2, declarePrimitives, applyPrimitive, lookupPrimEnv, } from "../tree/eval.js";
import { logger_log } from "../utils/logger.js";
import { assert } from "../utils/assert.js";
import { nilT, boolT, intT, strT, anyT, pairT, listT, funT, funPT, funDT, varT, voidT, ruleT, typeT, singleT, unionTypes, errorT, hdT, tlT, rangeT, unionT, intersectT, relcompT, applyT, singleTermVarT, singleTypeT, tuple2T, maybeT, charT, unknownT, recT, selfT, typeTupleMap, typeDom, typeRng, intersectTypes, typeRelComp1, typeHd, typeTl, typeElem, typeInverse, typeInverseApply, } from "../tree/types.js";
import { eVar } from "../syntax/expr.js";
// import { runtimeUtils, primitivesCore } from "../runtime/runtime-core.js"
let loopType = funPT("L", funT(voidT, unionTypes(tuple2T(singleT('break'), anyT), tuple2T(singleT('continue'), ruleT("domainT", [varT("L")])))), funPT("S", ruleT("domainT", [varT("L")]), hdT(tlT(ruleT("intersectT", [tuple2T(singleT('break'), anyT), ruleT("applyT", [varT("L"), varT("S")])])))));
let loop2Type = funPT("A", anyT, funPT("B", funT(varT("A"), unionT(tuple2T(singleT("break"), anyT), tuple2T(singleT("continue"), varT("A")))), hdT(tlT(intersectT(applyT(varT("B"), varT("A")), pairT(singleT("break"), anyT))))));
...
function mkTypeFunc2Prim(tyFun) {
    return (args) => {
        let [a, b] = args;
        let a1 = evalNode(a);
        let b1 = evalNode(b);
...
function unionTypeListPrim(args) {
    let [a] = args;
    let a1 = evalNode(a);
    let rT = voidT;
    let typeList = [];
    ...
    typeList.reverse().forEach((ty) => {
        rT = unionTypes(ty, rT);
    });
    return node(typeValue(rT)); // ruleT("unionT", [aT, bT]
}
function variantTypePrim3(args) {
    let [a, b] = args;
    let a1 = evalNode(a);
...
function tupleMapPrim(args) {
    let [f, t] = args;
    let f1 = evalNode(f);
    let t1 = evalNode(t);
    if (f1.tag === 'type' && t1.tag === 'type') {
        let ty = typeTupleMap(f1.type, t1.type);
        return node(typeValue(ty));
    }
    else {
...
let prim_hpsCallK = (args) => {
    let [result, handler] = args;
    return result;
};
let prim_hpsCall = (args) => {
    let [action, handler] = args;
    let k = node(primValue2("primHpsCallK", 2, []));
    let result = apply(apply(action, k), handler);
    return result;
};
let prim_hpsHandlerMk = (args) => {
...
const primTable = {
    ...
    "head": [1, mkProjecttionPrim((a) => a.head), funPT("X", pairT(anyT, anyT), ruleT("hdT", [varT("X")]))],
    "tail": [1, mkProjecttionPrim((a) => a.tail), funPT("X", pairT(anyT, anyT), ruleT("tlT", [varT("X")]))],
    "hd": [1, mkProjecttionPrim((a) => a.head), funPT("X", pairT(anyT, anyT), ruleT("hdT", [varT("X")]))],
    "tl": [1, mkProjecttionPrim((a) => a.tail), funPT("X", pairT(anyT, anyT), ruleT("tlT", [varT("X")]))],
    "if": [2, if2Prim2, funT(boolT, funPT("K", pairT(funT(nilT, anyT), pairT(funT(nilT, anyT), nilT)), ruleT("rangeT", [ruleT("elemT", [varT("K")])])))],
    "if2": [2, if2Prim2, funT(boolT, funPT("K", pairT(funT(nilT, anyT), pairT(funT(nilT, anyT), nilT)), ruleT("rangeT", [ruleT("elemT", [varT("K")])])))],
    ...
    "fix": [2, fixPrim2,
        funPT("F", funT(funT(anyT, voidT), ruleT('domainT', [varT('F')])), ruleT('rangeT', [varT('F')]))
    ],
    "fix2": [2, fixPrim2,
        funPT("F", funT(funT(anyT, voidT), ruleT('domainT', [varT('F')])), ruleT('rangeT', [varT('F')]))
    ],
    ...
    "ifNil": [2, mkIfPrim(a => a.tag === 'atomic' && a.value === null),
        funPT("V", anyT, funPT("K", pairT(funT(nilT, anyT), pairT(funT(ruleT("relcompT", [varT("V"), nilT]), anyT), nilT)), ruleT("unionT", [ruleT("rangeT", [ruleT("hdT", [varT("K")])]), ruleT("rangeT", [ruleT("hdT", [ruleT("tlT", [varT("K")])])])])))],
    "ifInt": [2, mkIfPrim(a => a.tag === 'atomic' && typeof a.value === "number"),
        funPT("V", anyT, funPT("K", pairT(funT(intT, anyT), pairT(funT(ruleT("relcompT", [varT("V"), intT]), anyT), nilT)), ruleT("unionT", [ruleT("rangeT", [ruleT("hdT", [varT("K")])]), ruleT("rangeT", [ruleT("hdT", [ruleT("tlT", [varT("K")])])])])))],
    "ifBool": [2, mkIfPrim(a => a.tag === 'atomic' && typeof a.value === "boolean"),
        funPT("V", anyT, funPT("K", pairT(funT(boolT, anyT), pairT(funT(relcompT(varT("V"), boolT), anyT), nilT)), unionT(applyT(hdT(varT("K")), boolT), applyT(hdT(tlT(varT("K"))), relcompT(varT("V"), boolT)))))],
    "ifStr": [2, mkIfPrim(a => a.tag === 'atomic' && typeof a.value === "string"),
        funPT("V", anyT, funPT("K", pairT(funT(strT, anyT), pairT(funT(relcompT(varT("V"), strT), anyT), nilT)), unionT(applyT(hdT(varT("K")), strT), applyT(hdT(tlT(varT("K"))), relcompT(varT("V"), strT)))))],
    "ifPair": [2, mkIfPrim(a => a.tag === 'pair'),
        funPT("V", anyT, funPT("K", pairT(funT(pairT(anyT, anyT), anyT), pairT(funT(relcompT(varT("V"), pairT(anyT, anyT)), anyT), nilT)), unionT(applyT(hdT(varT("K")), pairT(anyT, anyT)), applyT(hdT(tlT(varT("K"))), relcompT(varT("V"), pairT(anyT, anyT))))))],
    "ifType": [2, mkIfPrim(a => a.tag === 'type'),
        funPT("V", anyT, funPT("K", pairT(funT(typeT, anyT), pairT(funT(relcompT(varT("V"), typeT), anyT), nilT)), unionT(applyT(hdT(varT("K")), typeT), applyT(hdT(tlT(varT("K"))), relcompT(varT("V"), typeT)))))],
