...
import { feData_fromJson, feData_toJson, showValueFerrum2, primitivesCore, runtimeUtils } from "../runtime/runtime-core.js";
let console_log = console.log;
// let console_log = console.error
export function errorObjToData(err) {
    // TODO ? return a key-value list ?
    return JSON.stringify(err);
}
export const adaptReqResp0 = (rrFe) => {
    const [req, [kResp,]] = rrFe;
    const rrJs = [feData_toJson(req),
        (resp) => new Promise(async (ok, fail) => ok(adaptReqResp0(await kResp(feData_fromJson(resp)))))];
    return rrJs;
};
export const adaptReqResp = (rrFe) => {
    return new Promise((ok, fail) => ok(adaptReqResp0(rrFe)));
};
export let mkIoDoAsyncFe = (cmdLineArgs) => {
    const ioDo = mkIoDoAsyncJs(cmdLineArgs);
    let ioDoAsync = async (rrFe) => {
        const rrJs = adaptReqResp(rrFe);
        ioDo(rrJs);
    };
    return ioDoAsync;
};
export let mkIoDoAsyncJs = (cmdLineArgs) => {
    let ioTable = new Map;
    let maxIoId = 16;
    let nextIoId = 0;
    let ioState = new Map();
    ...
    let ioDoAsync = async (rr) => {
        ...
        switch (reqName) {
            ...
            case "httpServerListen": {
                const [, serverNum, host, port] = req;
                const serverObj = ioIdGet(serverNum, "server");
            ...
            case "httpServerSendResponse": {
                const [, reqRespNum, headers, body] = req;
...
function primJsEvalMaybe(input) {
    let result;
    try {
...
//# sourceMappingURL=runtime-nodejs.js.map