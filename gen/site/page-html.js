...
export function docToHtml(ctx, styleLines, bodyLines, doc) {
    function mod(module) {
    ...
    htmlBuild(styleLines, bodyLines, (s, h) => {
        ...
        d2h(doc, true);
        return;
        function d2h(doc, top = false) {
            // const claas: HtmlAttrs = top ? { class: textCol }  : {}
            const claas = top ? textCol : "";
...
export function pageToHtml(ctx, opts, page) {
    function mod(module) {
        // const srcFile = path.join(ctx.topDir, module)
    ...
    if (opts.importAppScripts === true || opts.importAppScripts === undefined && ctx.numApps > 0) {
        lines.push("<script type='module'>", `  import { setIo } from ${mod("/ts/gen/io/io.js")}`, `  import { mkIoBrowser } from ${mod("/ts/gen/io/io-browser.js")}`, `  setIo(mkIoBrowser(new URL('/', window.location.href)))`, "</script>");
    }
    ...
    if (opts.importReloadScript === true || opts.importReloadScript === undefined && ctx.numApps === 0) {
        lines.push(`<script type="module">`, `import ${mod("/ts/gen/site/page-reload.js")}`, `</script>`);
    }
